...
class RunZEO(zeoobject.ZEOObject):
    ...
    def get_params(self):
        ...
        desc = study["run_desc"]
        params = tuple(
            [v for e, v in desc.entries.items()
    ...
    def getFile(self, filename, cache="/tmp"):
        dest_path = os.path.join(
            cache, "BD-" + self.base.schema +
            "-cache", "run-{0}".format(self.id)
        )
    ...
    def addConfigFiles(self, file_list, regex_params=None):
        ...
        files_to_add = [
            conffile_zeo.addFile(fname, regex_params=regex_params,
                                 params=params_list)
    ...
    def pushVectorQuantity(self, vec, step, name, description=None):
        ...
        if name not in self.quantities:
            ...
            if (len(list_vec.shape) == 2 and
                list_vec.shape[1] != vec.shape[0]) or len(
                list_vec.shape
            ) == 1:
                list_vec = np.array([e for e in list_vec] + [vec],
                                    dtype=object)
            else:
