...
class AbstractBase(ABC):
    ...
    def _createParameterSpace(self, entries, entry_nb=0):
        ...
        for value in e:
            ...
            created_entries += self._createParameterSpace(tmp_entries,
                                                          entry_nb + 1)
    ...
    def createParameterSpace(
        ...
        for e in tqdm(space):
            ...
            nb_inserted += 1
            logger.debug(
                "insert job #{0}".format(nb_inserted) +
                ": " + str(tmp_job.entries)
            )
            self.insert(tmp_job)
    ...
    def __init__(self, read_only=False, connection=None,
                 truerun=False, **kwargs):
