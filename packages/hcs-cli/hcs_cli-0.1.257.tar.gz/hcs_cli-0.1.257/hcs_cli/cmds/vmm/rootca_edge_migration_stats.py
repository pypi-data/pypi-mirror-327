...
def stats(org: str, edge_ids: str, run_id: str, verbose: bool):
    ...
    logger.info(
        f"Getting migration stats for all templates' images & vms of org_id: {target_org_id}, edges: {edge_ids}"
    )
    logger.info("\n")
...
def get_all_template_images_and_vms_stats(org: dict, edge: dict, edge_stats: dict, verbose: bool):
            ...
            for agent in agents:
                if agent.get("name") == "HAI-Agent":
                    ...
                    if is_valid_agent_version(agent.get("agentVersion")):
                        is_valid_agent = True
                        # logger.info(f"hai agent version {agent.get('agentVersion')} is VALID")
                    break
        else:
            is_using_custom_vm_image = True
            # logger.warn(f"Template {template['id']} uses manually installed agent on the image. So check agent version on the template VMs")
    ...
    if len(unique_image_ids) > 0:
        logger.info(f"unique_image_ids: {unique_image_ids}\n")
        logger.info("Images stats:")
    ...
    if evaluate_edge_eligibility:
        ...
        for template_stats in all_template_stats:
            if (
                "is_vm_image_on_valid_agent_version" in template_stats.keys()
...
def send_edge_stats_event(
    ...
    data = {
        ...
        hoc_fields["edge_ca_label_on_pki"]: edge_stats.get("edge_ca_label_on_pki"),
    }
...
def send_template_stats_event(template_stats: dict, success: bool, verbose: bool):
    ...
    data = {
        ...
        hoc_fields["migrated_vms"]: template_stats.get("migrated_vms"),
        hoc_fields["migrated_vms_count"]: template_stats.get("migrated_vms_count"),
        hoc_fields["non_migrated_vms"]: template_stats.get("non_migrated_vms"),
