...
def createDatabase(db, rebuild=False):
    ...
    db.run('create index if not exists idx_val_code on t_e(code)')
    db.run('create index if not exists idx_val_date on t_e(date)')
...
def fix_dwjz(db):
    ...
    def get_dwjz_one(db, code):
        ...
        log.info('get_dwjz', code, len(rss), len(params))

    tp = ThreadPool(64)
    log.info('get_dwjz', 'begin')
    funds = db.run("select distinct code from t_e where dwjz is null order by code").get_rows()
    for fund in funds:
        if len(params) >= 1000000:
            tp.wait()
            num = db.run(sql, params)
            log.info('get_dwjz', num)
            params.clear()
        else:
            tp.run_wait(get_dwjz_one, (db, fund[0]))
    tp.wait()
    num = db.run(sql, params)
    log.info('get_dwjz', num)
    params.clear()
...
def get_zf(db):
    ...
    funds = db.run("select distinct code from t_e where zf is null and dwjz is not null order by code").get_rows()
    for fund in funds:
...
def get_val(db):
    ...
    funds = db.run("select distinct code from t_e where val is null and zf is not null order by code").get_rows()
    for fund in funds:
...
def get_hc(db):
    ...
    funds = db.run("select distinct code from t_e where hc is null and val is not null order by code").get_rows()
    for fund in funds:
...
def get_nh(db):
    ...
    def get_nh3n_one(db, code):
        ...
        for row in rows:
            ...
            try:
                val_future = db.run('select val from t_e where date >= ? and code = ? order by date limit 1', (date_future, code)).get_rows()[0][0]
                val_future = float(val_future)
                nh3n = round((val_future - val) / val * 100, 3)
            except:
                nh3n = None
            if nh3n is None:
                break
    ...
    def get_nh5n_one(db, code):
        ...
        for row in rows:
            ...
            try:
                val_future = db.run('select val from t_e where date >= ? and code = ? order by date limit 1', (date_future, code)).get_rows()[0][0]
                val_future = float(val_future)
                nh5n = round((val_future - val) / val * 100, 3)
            except:
                nh5n = None
            if nh5n is None:
                break
    ...
    funds = db.run("select distinct code from t_e where val is not null order by code").get_rows()
    for fund in funds:
