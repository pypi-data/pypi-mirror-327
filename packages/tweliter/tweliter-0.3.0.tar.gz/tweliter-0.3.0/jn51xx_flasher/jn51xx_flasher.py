...
from jn51xx_flasher.jn51xx_protocol import *  # Modified path
...
class Flasher:
    ...
    def writeFirmware(self, filename):
        ...
        # Flash data in reverse order # Modified for progress monitor
        #for i, addr in enumerate(range(start_addr, -1, -0x80)):
        #    self.writeFlash(addr, firmware[addr : addr + chunklen])
        #    chunklen = 0x80
        #    sys.stdout.write(
        #        f"\rWriting... {math.ceil(0x80 * i * 100 / firmware_size):03d}%"
        #    )
        #    sys.stdout.flush()
        #print()

        # Modified for pretty progress monitor
        with tqdm(total=firmware_size, unit='B',
                  unit_scale=True, desc="Writing") as pbar:
            for addr in range(start_addr, -1, -0x80):
                self.writeFlash(addr, firmware[addr : addr + chunklen])
    ...
    def verifyFirmware(self, filename):
        ...
        for addr in range(0, len(firmware), 0x80):
            ...
            if chunk != firmware[addr : addr + chunklen]:
                print(
                    f"Firmware verification failed: data different at addr {addr:08x}"
                )
                errors = True
