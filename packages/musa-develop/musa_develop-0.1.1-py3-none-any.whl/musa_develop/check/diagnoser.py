...
from musa_develop.utils import GPU_ARCH_MAP, GPU_TYPE_MAP, GPU_MEMORY_MAP
        ...
        if self._tag in self._version_requirements.yaml_data.keys():
            ...
            if (
                ...
                if not special_version_dict:
                    self._check_status[
                        module_name
                    ].solution += f"\n{' '*SOLUTION_INDENTATION_LENGTH}The current {module_name} version may not be an official release version. The version compatibility check has been skipped. If necessary, please manually check the version compatibility."
                else:
        ...
        if HostModule.Lspci.name in self._log:
            ...
            if lspci_info[2]:
                ...
                if gpu_type:
                    ...
                    lspci_dict.GPU_Type = "Unknown"
                    lspci_dict.status = Status.WARNING
                    lspci_dict.solution = "Unable to match GPU architecture."
    ...
    def diagnose_gmi(self) -> None:
        ...
        if DriverModule.Driver.name in self._log:
            ...
            if not driver_code and driver_out:
                ...
                gpu_standard_memory = GPU_MEMORY_MAP[
                    out_dict["Product Name"][0]
                ]  # eg: S80 --> 16384
                gpu_actual_memory = [
                ...
                if gpu_standard_memory != gpu_actual_memory[0]:
                    gmi_dict.status = Status.WARNING
    ...
    def diagnose_ib(self) -> None:
        if IBModule.ibstat.name in self._log:
            ...
            if "not found" in ibstat_err:
                ...
                ib_dict.solution = "The ibstat command is not recognized, please install it: \n \t sudo apt install infiniband-diags"
    ...
    def _diagnose_package(
        ...
        if pip_version[0] and not pip_version[2]:
            ...
            if run_demo_log[2]:
                ...
                check_status.solution = f"The {name} demo test failed, please manually check {name} is not working properly."
            else:
    ...
    def diagnose_mttransformer(self) -> None:
        ...
        if vLLMModule.MTTransformer.name in self._log:
            ...
            if mtt_version[0]:
                ...
                mtt_dict.version = mtt_version[0].split()[-1]
            else:
