...
class GetAuthorizationUrl(BaseModel):
    ...
    """ # noqa: E501
    client_id: StrictStr = Field(description="OAuth client ID provided by the third-party service.")
    scopes: List[StrictStr] = Field(description="List of OAuth scopes to request.")
    redirect_uri: StrictStr = Field(description="URL where the user will be redirected after authorization. Must match the connector settings.")
    state: StrictStr = Field(description="State parameter for security validation.")
    form_data: Optional[Dict[str, StrictStr]] = Field(default=None, description="Form data to include in the authorization request.")
    __properties: ClassVar[List[str]] = ["client_id", "scopes", "redirect_uri", "state", "form_data"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of GetAuthorizationUrl from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of GetAuthorizationUrl from a dict"""
        if obj is None:
            return None
