...
class CreateAccountEntitlement(BaseModel):
    """
    Required entitlement configuration for assigning permissions during account creation
    """ # noqa: E501
    integration_specific_resource_id: Optional[StrictStr] = Field(default=None, description="The unique identifier for the resource in the third-party system that the entitlement will be assigned to. For global resources (e.g. global roles), this should be an empty string or leave blank. For scoped resources (e.g. workspace-specific roles), this should be the resource ID (e.g. workspace ID).")
    entitlement_type: StrictStr = Field(description="Should match an entitlement type returned by this connector's info response")
    integration_specific_id: StrictStr = Field(description="The unique identifier for the specific required entitlement in the third-party system")
    __properties: ClassVar[List[str]] = ["integration_specific_resource_id", "entitlement_type", "integration_specific_id"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CreateAccountEntitlement from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CreateAccountEntitlement from a dict"""
        if obj is None:
            return None
