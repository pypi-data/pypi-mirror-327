...
class HandleAuthorizationCallback(BaseModel):
    ...
    redirect_uri_with_code: StrictStr = Field(
        description="The redirect URI containing the authorization code returned by the OAuth provider."
    )
    state: Optional[StrictStr] = Field(
        default=None, description="(Optional) A state parameter for security validation."
    )
    code_verifier: Optional[StrictStr] = Field(
        default=None,
        description="A code verifier for PKCE. This is returned from the get_authorization_url operation if PKCE is enabled.",
    )
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={"x-capability-category": "authorization"},
    )
