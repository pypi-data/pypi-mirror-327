...
from connector.generated.models.auth_model import AuthModel
from connector.generated.models.credential_config_input_model import CredentialConfigInputModel
from typing import Optional, Set
from typing_extensions import Self

class CredentialConfig(BaseModel):
    """
    App authentication model, allowing an app to have multiple authentication schemas, identified by a string ID.
    """ # noqa: E501
    id: StrictStr = Field(description="The ID of the authentication schema. Must be unique within one app - no two AppAuths should share the same ID.")
    type: AuthModel = Field(description="The authentication type, simplified identificator.")
    description: StrictStr = Field(description="The markdown description of the authentication. Used primarily for instructions provided to customers.")
    optional: Optional[StrictBool] = Field(default=False, description="Denotes whether this credential is optional, for example, if the integration can function just fine with one credential, and another credential is not strictly required, eg. the app will operate under a limited scope.")
    input_model: Optional[CredentialConfigInputModel] = None
    __properties: ClassVar[List[str]] = ["id", "type", "description", "optional", "input_model"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of input_model
        if self.input_model:
            _dict['input_model'] = self.input_model.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "input_model": CredentialConfigInputModel.from_dict(obj["input_model"]) if obj.get("input_model") is not None else None
        })
        return _obj
