...
CREDENTIALCONFIGINPUTMODEL_ANY_OF_SCHEMAS = ["BasicCredential", "JWTCredential", "OAuthClientCredential", "OAuthCredential", "TokenCredential"]

class CredentialConfigInputModel(BaseModel):
    """
    The input model expected by the app to authenticate against the 3rd party service. This is provided so that the default model can be overriden by the app.
    """

    # data type: OAuthCredential
    anyof_schema_1_validator: Optional[OAuthCredential] = None
    # data type: OAuthClientCredential
    anyof_schema_2_validator: Optional[OAuthClientCredential] = None
    # data type: BasicCredential
    anyof_schema_3_validator: Optional[BasicCredential] = None
    # data type: TokenCredential
    anyof_schema_4_validator: Optional[TokenCredential] = None
    # data type: JWTCredential
    anyof_schema_5_validator: Optional[JWTCredential] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[BasicCredential, JWTCredential, OAuthClientCredential, OAuthCredential, TokenCredential]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "BasicCredential", "JWTCredential", "OAuthClientCredential", "OAuthCredential", "TokenCredential" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = CredentialConfigInputModel.model_construct()
        error_messages = []
        ...
        # validate data type: OAuthClientCredential
        if not isinstance(v, OAuthClientCredential):
            error_messages.append(f"Error! Input type `{type(v)}` is not `OAuthClientCredential`")
        else:
            return v
        ...
        # validate data type: TokenCredential
        if not isinstance(v, TokenCredential):
            error_messages.append(f"Error! Input type `{type(v)}` is not `TokenCredential`")
        else:
            return v

        # validate data type: JWTCredential
        if not isinstance(v, JWTCredential):
            error_messages.append(f"Error! Input type `{type(v)}` is not `JWTCredential`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in CredentialConfigInputModel with anyOf schemas: BasicCredential, JWTCredential, OAuthClientCredential, OAuthCredential, TokenCredential. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_2_validator: Optional[OAuthClientCredential] = None
        try:
            instance.actual_instance = OAuthClientCredential.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_3_validator: Optional[BasicCredential] = None
        ...
        # anyof_schema_4_validator: Optional[TokenCredential] = None
        try:
            instance.actual_instance = TokenCredential.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_5_validator: Optional[JWTCredential] = None
        try:
            instance.actual_instance = JWTCredential.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into CredentialConfigInputModel with anyOf schemas: BasicCredential, JWTCredential, OAuthClientCredential, OAuthCredential, TokenCredential. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BasicCredential, JWTCredential, OAuthClientCredential, OAuthCredential, TokenCredential]]:
