...
def post_runs_close(
    *,
    ...
    _query_params: dict[str, Any] = {"skip_tests": int(True)}
    response = _request(method="POST", resource_name=f"runs/{run_id}/close", query_params=_query_params)
    try:
        _ = _parse_response(response)
    except DBNLAPIError as e:
        # ENG-1483: skip tests will be removed from API soon
        if "skip_tests" in str(e) and "Unknown field." in str(e):
...
def _maybe_warn_invalid_version() -> None:
    ...
    if response.status_code != 200:
        raise DBNLVersionError(f"Failed to fetch OpenAPI spec: {response.status_code}")
    ...
    try:
        ...
        raise DBNLVersionError("Invalid OpenAPI spec JSON") from e
