...
from dbnl.errors import DBNLVersionError
...
def check_version_compatibility(api_version: Optional[str]) -> None:
    ...
    :raises DBNLVersionError: If versions are incompatible
    """
    if not api_version:
        raise DBNLVersionError("Could not determine API version from OpenAPI spec")
    ...
    try:
        ...
        if api_version == "v0":
            ...
            raise DBNLVersionError(f"Invalid API version format: {api_version}")
