...
from fi.integrations.otel.types import FiSpanKindValues, SpanAttributes
...
def _get_input_value(method: Callable[..., Any], *args: Any, **kwargs: Any) -> str:
    ...
    tool = kwargs.get("calling")
    if tool and isinstance(tool, ToolCalling):
    ...
    if args and args[0] and isinstance(args[0], Agent):
        return safe_json_dumps(
            {
                "role": args[0].role,
                "goal": args[0].goal,
                "backstory": args[0].backstory,
            },
            cls=SafeJSONEncoder,
        )
