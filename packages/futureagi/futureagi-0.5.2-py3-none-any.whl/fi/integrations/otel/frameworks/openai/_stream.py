...
from fi.integrations.otel.frameworks.openai._span_io_handler import (
    add_stream_output_to_span,
)
from fi.integrations.otel.frameworks.openai._utils import _finish_tracing
from fi.integrations.otel.frameworks.openai._with_span import _WithSpan
...
class _Stream(ObjectProxy):  # type: ignore
    ...
    def __init__(
        ...
        self._self_accumulated_content = ""
    ...
    def _process_chunk(self, chunk: Any) -> None:
        # Extract content from chunk if it has the expected structure
        content = ""
        if (
            ...
            # Access content directly from the delta object if it exists
            content = getattr(chunk.choices[0].delta, "content", "") or ""

        if content:
            self._self_accumulated_content += content
            print(f"Accumulated content so far: {self._self_accumulated_content}")
        ...
        try:
            add_stream_output_to_span(self._self_with_span.span, chunk)
        except Exception:
            logger.exception("Failed to add stream output to span")

    def _finish_tracing(
        self,
        status: Optional[trace_api.Status] = None,
    ) -> None:
        # Set the final accumulated content as an attribute on the span
        if self._self_accumulated_content:
            self._self_with_span.set_attribute(
                "fi.llm.output", self._self_accumulated_content
            )
