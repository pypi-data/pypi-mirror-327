...
from fi.integrations.otel.types import SpanAttributes
...
def _process_input_data(input_data: Any, span: _WithSpan) -> None:
    ...
    if isinstance(input_data, list):
        ...
        if input_content:
            span.set_attribute(SpanAttributes.INPUT_VALUE, str(input_content))
        if input_images:
            span.set_attribute(SpanAttributes.INPUT_IMAGES, str(input_images))
        if eval_input:
            eval_input_str = " | ".join(map(str, eval_input))
            ...
            if hasattr(output_data, "choices") and output_data.choices:
                ...
                if hasattr(first_choice, "message"):
                    ...
                    if hasattr(message, "tool_calls") and message.tool_calls:
                        ...
                        output_content = " | ".join(tool_calls)
