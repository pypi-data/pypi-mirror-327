...
from fi.integrations.otel.frameworks.anthropic._utils import (
...
class _Stream(ObjectProxy):  # type: ignore
    ...
    def __iter__(self) -> Iterator["Completion"]:
        ...
        try:
            ...
            raise
        print("Stream iteration completed successfully")
        status = trace_api.Status(
    ...
    def _finish_tracing(
        ...
        _finish_tracing(
            ...
            has_attributes=_ResponseExtractor(
...
class _MessagesStream(ObjectProxy):  # type: ignore
    ...
    def __init__(
        ...
        print("Initializing _MessagesStream wrapper")
        super().__init__(stream)
        self._response_accumulator = _MessageResponseAccumulator()
        self._with_span = with_span

    def __iter__(self) -> Iterator["RawMessageStreamEvent"]:
        print("Starting synchronous message stream iteration")
        try:
            for item in self.__wrapped__:
                print(f"Processing message stream chunk: {type(item)}")
                self._response_accumulator.process_chunk(item)
                yield item
        except Exception as exception:
            print(
                f"Error in message stream iteration: {type(exception).__name__}: {exception}"
            )
            status = trace_api.Status(
                status_code=trace_api.StatusCode.ERROR,
                description=f"{type(exception).__name__}: {exception}",
            ...
            self._finish_tracing(status=status)
            raise
        print("Message stream iteration completed successfully")
        status = trace_api.Status(
            status_code=trace_api.StatusCode.OK,
        )
        self._finish_tracing(status=status)
...
class _MessageResponseAccumulator:
    ...
    def __init__(self) -> None:
        ...
        self._current_content_block_type: Union["TextBlock", "ToolUseBlock", None] = (
            None
        )
        self._values = _ValuesAccumulator(
    ...
    def process_chunk(self, chunk: "RawContentBlockDeltaEvent") -> None:
        print(f"Processing message chunk of type: {type(chunk)}")
        ...
        if isinstance(chunk, RawMessageStartEvent):
            print(
                f"Processing message start event. Message index: {self._current_message_idx + 1}"
            )
            self._current_message_idx += 1
            value = {
                "messages": {
            ...
            print(
                f"Processing content block start event. Type: {type(chunk.content_block)}"
            )
            self._current_content_block_type = chunk.content_block
        elif isinstance(chunk, RawContentBlockDeltaEvent):
            print(
                f"Processing content block delta event. Current block type: {type(self._current_content_block_type)}"
            )
            ...
            print("Processing message delta event")
            value = {
                "messages": {
                    "index": str(self._current_message_idx),
...
class _MessageResponseExtractor:
    ...
    def get_attributes(self) -> Iterator[Tuple[str, AttributeValue]]:
        ...
        if message_content:
            yield from _as_output_attributes(
                _ValueAndType(message_content, FiMimeTypeValues.TEXT)
            )
        else:
            json_string = safe_json_dumps(result)
            yield from _as_output_attributes(
