...
class _WithTracer(ABC):
    ...
    def _start_as_current_span(
        ...
        print(f"Starting span: {span_name} with attributes: {attributes}")
        try:
            ...
            print(f"Successfully created span: {span}")
...
class _CompletionsWrapper(_BaseWrapper):
    ...
    def __call__(
        ...
        with self._start_as_current_span(
            ...
            try:
                print("Calling wrapped completion function")
            ...
            print("Completion successful, processing response")
        ...
        with self._start_as_current_span(
            ...
            try:
                ...
                if hasattr(response, "content"):
                    print("Processing response content", response.content)
...
def _get_outputs(response: "BaseModel") -> Iterator[Tuple[str, Any]]:
    print(f"Getting outputs from response: {response}")
...
def _get_llm_tools(
    ...
    print(f"Processing LLM tools from parameters: {invocation_parameters}")
    if isinstance(tools := invocation_parameters.get("tools"), list):
        ...
        for tool_index, tool_schema in enumerate(tools):
            print(f"Processing tool {tool_index}: {tool_schema}")
...
def _get_llm_input_messages(messages: List[Dict[str, str]]) -> Any:
    print(f"Processing input messages: {len(messages)} messages")
    ...
    for i in range(len(messages)):
        ...
        if content := messages[i].get("content"):
            if isinstance(content, str):
                print(f"Processing string content: {content[:100]}... (truncated)")
                yield f"{LLM_INPUT_MESSAGES}.{i}.{MESSAGE_CONTENT}", content
            elif isinstance(content, list):
                print(f"Processing content blocks: {len(content)} blocks")
                for block in content:
                    if isinstance(block, ToolUseBlock):
                        print(f"Processing ToolUseBlock: {block}")
                        ...
                        print(
                            f"Processing TextBlock: {block.text[:100]}... (truncated)"
                        )
                        yield f"{LLM_INPUT_MESSAGES}.{i}.{MESSAGE_CONTENT}", block.text
                    elif isinstance(block, dict):
                        print(f"Processing dict block: {block}")
...
def _get_invocation_parameters(kwargs: Mapping[str, Any]) -> Any:
    print("Extracting invocation parameters")
    invocation_parameters = {}
    for key, value in kwargs.items():
        if _validate_invocation_parameter(key):
            print(f"Adding valid parameter: {key}")
