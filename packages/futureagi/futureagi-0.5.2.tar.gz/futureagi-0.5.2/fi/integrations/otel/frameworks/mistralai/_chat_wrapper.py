...
from fi.integrations.otel.frameworks.mistralai._request_attributes_extractor import (
    _RequestAttributesExtractor,
)
from fi.integrations.otel.frameworks.mistralai._response_accumulator import (
    _ChatCompletionAccumulator,
)
from fi.integrations.otel.frameworks.mistralai._response_attributes_extractor import (
    _ResponseAttributesExtractor,
    _StreamResponseAttributesExtractor,
)
from fi.integrations.otel.frameworks.mistralai._stream import _AsyncStream, _Stream
from fi.integrations.otel.frameworks.mistralai._utils import (
    _as_input_attributes,
    _finish_tracing,
    _io_value_and_type,
)
from fi.integrations.otel.frameworks.mistralai._with_span import _WithSpan
    ...
    def __call__(
        ...
        try:
            request_parameters = self._parse_args(
                signature(wrapped), instance, *args, **kwargs
            )
        except Exception:
        ...
        with self._start_as_current_span(
            ...
            try:
                ...
                logger.exception(
                    f"Failed to finish tracing for response of type {type(response)}"
                )
...
class _ResponseAttributes:
    ...
    def get_attributes(self) -> Iterator[Tuple[str, AttributeValue]]:
        if hasattr(self._response, "model_dump_json") and callable(
