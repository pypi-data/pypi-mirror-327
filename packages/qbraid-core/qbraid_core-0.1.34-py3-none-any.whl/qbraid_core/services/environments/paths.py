...
def installed_envs_data() -> tuple[dict[str, Path], dict[str, str]]:
    ...
    def _process_entry(entry, installed, aliases):
            ...
            if state_json_path.exists():
                try:
                    with open(state_json_path, "r", encoding="utf-8") as file:
                        data = json.load(file)
                    if data.get("name") in aliases:
                        # shell_prompt, if present, is set as the alias for envs

                        # data[name] contains the alias for the env

                        # if shell_prompt is same for 2 envs with different
                        # names, the new env will overwrite the old one

                        # so, we use the slug as the alias if we already have the
                        # name in the aliases dict
    ...
    for env_path in qbraid_env_paths:
        for entry in env_path.iterdir():
            _process_entry(entry, installed, aliases)
