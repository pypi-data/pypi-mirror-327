    ...
    with open(s_function_file_name,"w") as out:
      ...
      for e in artifacts:
          if isinstance(e, SourceArtifact):
            out.write(f"""
          if (!ssWriteRTWStrParam(S, "CustomSource", "{e.relative}")) {{
              ssSetErrorStatus(S, "Error writing Custom Source file {e.relative} to RTW.");
              return;
          }}
          """)
          if isinstance(e, HeaderDirectory):
            out.write(f"""
          if (!ssWriteRTWStrParam(S, "CustomInclude", "{e.dir}")) {{
              ssSetErrorStatus(S, "Error writing Custom Include directory {e.dir} to RTW.");
              return;
          }}
          """)
          if isinstance(e, LibraryArtifact):
            out.write(f"""
          if (!ssWriteRTWStrParam(S, "CustomLibrary", "lib{e.basename}.a")) {{
              ssSetErrorStatus(S, "Error writing Custom Library lib{e.basename}.a to RTW.");
              return;
          }}
          """)
