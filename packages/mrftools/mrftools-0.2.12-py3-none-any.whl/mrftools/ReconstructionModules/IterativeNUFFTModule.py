...
class IterativeNUFFTModule(NUFFTModule):
    ...
    def L2Gradient(self, x):
        ...
        """
        with torch.no_grad():
            E_x = self.UnapplySVDCompression(self.PerformNUFFTs(self.PerformThroughplaneFFT(self.ExpandCoils(x)), self.useDCFInIterations))
            E_x[self.rawDataMask] = E_x[self.rawDataMask] - self.rawData
            E_x[self.rawDataMask == 0] = 0
            result = 2 * self.CombineCoils(self.PerformThroughplaneIFFT(self.PerformAdjointNUFFTs(self.ApplySVDCompression(E_x),self.useDCFInIterations)))
            return result
    ...
    def Prep(self, rawData):
        ...
        coilImageData = self.PerformThroughplaneIFFT(self.PerformAdjointNUFFTs(self.ApplySVDCompression(self.rawData), applyDCF=(self.initializeWithDCF)))
        imageDataTemp, coilmapsTemp = self.PerformWalshCoilCombination(coilImageData)
        print ("Coil Image Data:", coilImageData.shape, "Coilmaps:", coilmapsTemp.shape, "Combined:", imageDataTemp.shape)
    ...
    def Run(self):
        ...
        while((iteration < self.maxIterations) and (dxNorm > self.gradTolerance)): 
            ...
            print(iteration, f0.item(), t0, dxNorm.item(), restartScore)
    ...
    def ProcessKspaceToImage(self, inputData):
        ...
        with torch.no_grad():
            ...
            outputData = outputData.moveaxis(0,-1) # put into ImageData [x,y,z,t,coil] format
