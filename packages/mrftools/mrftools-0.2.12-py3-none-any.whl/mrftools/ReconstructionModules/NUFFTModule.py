...
class NUFFTModule(ReconstructionModule):
    ...
    def PerformAdjointNUFFTs(self, input): 
        ...
        """
        with torch.no_grad():
            adjoint_nufft = tkbn.KbNufftAdjoint(im_size=(self.matrixX, self.matrixY), grid_size=(self.matrixX*2, self.matrixY*2), numpoints=self.numNearestNeighbors).to(self.device)
            input = torch.moveaxis(input,-1,0) 
            numImages = input.shape[0]
            numCoils = input.shape[1]
            numPartitions = input.shape[2]
            output = torch.zeros(numImages, numCoils, self.matrixX, self.matrixY, numPartitions, dtype=input.dtype)
            for partition in torch.arange(0,numPartitions):
                ...
                nufftResult = adjoint_nufft(readout_device * self.dcf, self.ktraj)
                output[:,:,:,:, partition] = nufftResult
                del readout_device, nufftResult
            output = torch.moveaxis(torch.moveaxis(output, 0, -1), 0, -1)
            return output
    
    def __init__(self, reconstructionParameters, inputType:ReconstructionModuleIOType=ReconstructionModuleIOType.KSPACE, outputType:ReconstructionModuleIOType=ReconstructionModuleIOType.IMAGE, ktraj=None, dcf=None, numNearestNeighbors=5, device=None):
        """
