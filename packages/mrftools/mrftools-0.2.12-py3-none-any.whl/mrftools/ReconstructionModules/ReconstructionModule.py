...
class ReconstructionModule:
    ...
    def Process(self, input:DataStruct):
        ...
        if(input.dataType == self.inputType):
            ...
            elif(self.inputType == ReconstructionModuleIOType.MAP and self.outputType == ReconstructionModuleIOType.IMAGE):
                return self.ProcessMapToImage(input.data)
            else:
                print("Unknown IO Type Pair:", self.inputType, "|", self.outputType)
                return None
        else:
