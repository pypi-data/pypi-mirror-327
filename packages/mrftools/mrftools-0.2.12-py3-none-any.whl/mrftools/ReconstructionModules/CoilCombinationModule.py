...
def calculateCoilmapsWalsh(img, smoothing=5, niter=3, device=torch.device("cpu")):
    ...
    with torch.no_grad():
        ...
        print("Coilmap shape initial:", csm.shape)
        csm = torch.moveaxis(csm,0,-1)
        print("Coilmap shape final:", csm.shape)
        return (csm, rho)
...
class CoilCombinationModule(ReconstructionModule):
    ...
    def PerformWalshCoilCombination(self, input, kernelSize=(5,5,1), niter=5):
        ...
        Args:
            input (torch.Tensor): Input complex images of shape [x,y,z,svdComponent, coil]
        ...
        Returns:
            torch.Tensor: Combined complex image of shape [x,y,z,svdComponent]
        """
        with torch.no_grad():
            shape = np.shape(input)
            combinedImageData = torch.zeros((shape[0], shape[1], shape[2], shape[3]), dtype=torch.complex64)
            coil_map, rho = calculateCoilmapsWalsh(input[:,:,:,0,:], smoothing=kernelSize, niter=niter, device=self.device)
            self.coil_map = coil_map
            for svdComponent in np.arange(0,shape[3]):
                im = (input[:, :, :, svdComponent, :]).to(self.device)
                combinedImageData[:, :, :, svdComponent] = torch.sum((im * torch.conj(coil_map)), axis=3)
                del im
    ...
    def ProcessImageToImage(self, inputData):
        ...
        with torch.no_grad():
            if(self.mode == "walsh"):
                outputData = torch.moveaxis(self.PerformWalshCoilCombination(inputData),-1,0)
