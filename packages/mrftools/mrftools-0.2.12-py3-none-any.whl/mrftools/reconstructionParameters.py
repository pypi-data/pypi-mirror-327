...
class ReconstructionParameters:
    ...
    def Run(self, input):
        ...
        with torch.no_grad():
            ...
            for module in self.modules:
                startTime = time.time()
                print(module.__class__.__name__, " | ", module.inputType, "->", module.outputType)
    ...
    def append(self, module_class, **kwargs):
        """
        Append a new module to the reconstruction pipeline.

        Args:
            module_class (type): The class of the module to be appended.
            **kwargs: Additional keyword arguments for the module initialization.
        """
        # Initialize the module with the reconstructionParameter set to self
        module = module_class(reconstructionParameters=self, **kwargs)
        
        # Set the inputType of the new module to the outputType of the last module
        if self.modules:
            last_module = self.modules[-1]
            module = module_class(reconstructionParameters=self, inputType = last_module.outputType, **kwargs)
        else:
            module = module_class(reconstructionParameters=self, **kwargs)

        # Append the initialized module to the modules list
