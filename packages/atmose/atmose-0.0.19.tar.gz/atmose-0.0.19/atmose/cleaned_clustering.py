...
class HDBSCANConfig:
    ...
    outlier_threshold: int = 90  # Percentile threshold for outlier detection.
...
class PCAConfig:
    ...
    """
    target_evr: float | None = 0.9  # Target explained variance ratio.
...
class ClusteringEngine:
    ...
    def _create_branch_params(self, trial):
        ...
        Returns:
            dict: Dictionary with branch detection parameters:
                - min_branch_size: int, sampled from branch_config.min_cluster_size_min to branch_config.min_cluster_size_max.
        ...
        """
        branch_params = {
            "min_branch_size": trial.suggest_int("branch_min_cluster_size", self.branch_config.min_cluster_size_min, self.branch_config.min_cluster_size_max),
    ...
    def _pca_preprocess(self, df: pd.DataFrame):
        ...
        #----------------------
        # Re-fit PCA with the best number of components and transform the data.
        #----------------------
        ...
        self.logger.info("PCA preprocessing complete: selected n_components=%d with EVR=%.4f",
                         best_n_components, float(np.sum(pca_model.explained_variance_ratio_)))
        #----------------------
    ...
    def _final_clustering(self, best_umap, best_hdbscan, embeddings, best_trial):
        """
        ...
        if self.branch_config.enabled:
            ...
            if best_trial is not None:
                branch_min_cluster_size = best_trial.params.get("branch_min_cluster_size", self.branch_config.min_cluster_size_min)
                branch_selection_persistence = best_trial.params.get("branch_selection_persistence", self.branch_config.selection_persistence_min)
                branch_label_sides = best_trial.params.get("branch_label_sides_as_branches", self.branch_config.label_sides_as_branches)
            else:
                branch_min_cluster_size = self.branch_config.min_cluster_size_min
            ...
            #----------------------
            core_threshold = (100 - self.hdbscan_config.outlier_threshold) / 100.0
            core_flags = membership >= core_threshold
            #----------------------
            # No direct outlier score available when using BranchDetector.
            #----------------------
            ...
            threshold_val = np.percentile(outlier_scores_final, self.hdbscan_config.outlier_threshold)
...
def run_clustering(
    ...
    try:
        ...
        branch_config = BranchDetectionConfig(
            ...
            min_cluster_size_min=branch_min_cluster_size_min,
            min_cluster_size_max=branch_min_cluster_size_max,
