...
def main(args):
    model_and_co = setup(args.batch_size)
    train(args.epochs, *model_and_co)
...
class WikiHow(Dataset):
    def __init__(self, tokenizer, dataset, num_samples):
        self.dataset = dataset.select(list(range(0, num_samples)))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='T5 training with FlexDeMo')
