...
class Parameter:
    ...
    def set_default(self):
        ...
        if not self.init_non_exo_in_use_stock:
            self.init_non_exo_in_use_stock = (
                pd.DataFrame(
                    index=pd.MultiIndex([[]] * 2, [[]] * 2, names=["location", "object"]),
                    columns=clmn,
                ),
            )
        if not self.lifetime_mean_value:
            self.lifetime_mean_value = (
                pd.DataFrame(
                    index=pd.MultiIndex([[]] * 2, [[]] * 2, names=["location", "object"]),
                    columns=clmn,
                ),
            )
        if not self.lifetime_standard_deviation:
            self.lifetime_standard_deviation = (
                pd.DataFrame(
                    index=pd.MultiIndex([[]] * 2, [[]] * 2, names=["location", "object"]),
                    columns=clmn,
                ),
            )
        if not self.control_flow_shares:
            self.control_flow_shares = (
                pd.DataFrame(
                    index=pd.MultiIndex([[]] * 3, [[]] * 3, names=["location", "object", "process"]),
                    columns=clmn,
                ),
            )
        if not self.collection_rate:
        ...
        if not self.recycling_flow_shares:
            self.recycling_flow_shares = (
                pd.DataFrame(
                    index=pd.MultiIndex([[]] * 3, [[]] * 3, names=["location", "object", "process"]),
                    columns=clmn,
                ),
            )
        if not self.process_shares:
            self.process_shares = (
                pd.DataFrame(
                    index=pd.MultiIndex([[]] * 3, [[]] * 3, names=["location", "process", "object"]),
                    columns=clmn,
                ),
            )
        if not self.process_max_capacity:
            self.process_max_capacity = (
                pd.DataFrame(
                    index=pd.MultiIndex([[]] * 3, [[]] * 3, names=["location", "process", "object"]),
                    columns=clmn,
                ),
            )
        if not self.process_reference_capacity:
        ...
        if not self.thermodynamic_process:
            self.thermodynamic_process = (
                pd.DataFrame(
                    index=pd.MultiIndex([[]] * 3, [[]] * 3, names=["location", "process", "object"]),
                    columns=clmn,
                ),
            )
        if not self.object_efficiency:
            self.object_efficiency = (
                pd.DataFrame(
                    index=pd.MultiIndex(
                        [[]] * 4,
                        [[]] * 4,
        ...
        if not self.control_flow_trade:
            self.control_flow_trade = (
                pd.DataFrame(
