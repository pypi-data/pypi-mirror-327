...
class _MaterPreProcess:
    ...
    def _format_inputs(self, input_file: str):
        ...
        for input in input_dict:
            ...
            # Convert to datetime
            df.columns = pd.to_datetime(df.columns.astype(str), errors="coerce")
    ...
    def _outputs_initialization(self):
        ...
        if self.inputs.init_non_exo_in_use_stock.empty:
            ...
            df = self.inputs.init_non_exo_in_use_stock[self.simulation_start_time - self.offset].combine_first(
                self.inputs.exogenous_stock[self.simulation_start_time - self.offset]
            )
        # in-use stock
        ...
        df = _exponential_decay(self.inputs.lifetime_mean_value, self.simulation_start_time - self.offset, self.offset)
