...
def plotPotential(qcl: QCLayers, axes: Axes = None,
    ...
    highlights = [qcl.xLayerMask(n) for n, ar in enumerate(qcl.layerARs) if ar]
    if highlights:
        xNonARs = np.bitwise_and.reduce(highlights)
    else:
        xNonARs = np.ones(qcl.xPoints.shape, dtype=bool)
