...
class SysConfig:
    ...
    @classmethod
    def get_list(cls, section_name : str) -> List[Any]:
        list_ = cls.__get_instance().config.get(section_name)
        return list_ if isinstance(list_, list) and list_ else []

    @classmethod
    def get_map(cls, section_name : str) -> Dict[str, Any]:
        map_ = cls.__get_instance().config.get(section_name)
