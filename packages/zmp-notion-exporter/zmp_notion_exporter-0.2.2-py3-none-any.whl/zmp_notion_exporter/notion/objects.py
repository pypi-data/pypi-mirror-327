...
class BulletedListItem(BaseBlock):
    ...
    def generate_tag(
        ...
        if rendering_mode in [RenderingMode.MARKDOWN, RenderingMode.MARKDOWNX]:
            return f"{self.indent_space}- {make_markdown_one_line(self.rich_text, rendering_mode)}\n\n"
        elif rendering_mode == RenderingMode.HTML:
            return f"{self.indent_space}<li>{make_markdown_one_line(self.rich_text, rendering_mode)}</li>\n"
...
class Callout(BaseBlock):
    ...
    def generate_tag(
        ...
        emoji = self.icon.emoji if isinstance(self.icon, Emoji) else self.icon.file.url
        if rendering_mode == RenderingMode.MARKDOWN:
            return f"{self.indent_space} {emoji} {make_markdown_one_line(self.rich_text, rendering_mode)}\n\n"
            ...
            markdownx += "\n"
            markdownx += f"{make_markdown_one_line(self.rich_text, rendering_mode)}\n"
            markdownx += "\n"
            markdownx += ":::\n\n"
            return markdownx
        elif rendering_mode == RenderingMode.HTML:
            ...
            html += f"{make_markdown_one_line(self.rich_text, rendering_mode)}</span>\n"
...
class Code(BaseBlock):
    ...
    def generate_tag(
        ...
        if rendering_mode == RenderingMode.MARKDOWN:
            ...
            markdown += f"{make_markdown_one_line(self.rich_text, rendering_mode)}\n"
            markdown += "```\n\n"
            return markdown
        elif rendering_mode == RenderingMode.MARKDOWNX:
            ...
            html += make_markdown_one_line(self.rich_text, rendering_mode)
            html += "</code></pre>\n"
            return html
...
class XFile(BaseBlock):
    ...
    def generate_tag(
        ...
        if rendering_mode in [RenderingMode.MARKDOWN, RenderingMode.MARKDOWNX]:
            return f"{self.indent_space}[{self.name}]({filepath})\n\n"
        elif rendering_mode == RenderingMode.HTML:
            return f"{self.indent_space}<a href='{filepath}'>{self.name}</a>\n"
...
class Heading(BaseBlock):
    ...
    def generate_alink_tag(self, title: str) -> str:
        if title == "":
            return ""
...
class Heading1(Heading):
    def generate_tag(
        ...
        if rendering_mode == RenderingMode.MARKDOWN:
            ...
            title = make_markdown_one_line(self.rich_text, rendering_mode)
            return f"{self.indent_space}# {title} {self.generate_alink_tag(title)}\n\n"
        elif rendering_mode == RenderingMode.HTML:
            return f"{self.indent_space}<h1>{make_markdown_one_line(self.rich_text, rendering_mode)}</h1>\n"
...
class Heading2(Heading):
    def generate_tag(
        ...
        if rendering_mode == RenderingMode.MARKDOWN:
            ...
            return f"{self.indent_space}<h2>{make_markdown_one_line(self.rich_text, rendering_mode)}</h2>\n"
...
class Heading3(Heading):
    def generate_tag(
        ...
        if rendering_mode == RenderingMode.MARKDOWN:
            ...
            title = make_markdown_one_line(self.rich_text, rendering_mode)
            return (
                f"{self.indent_space}### {title} {self.generate_alink_tag(title)}\n\n"
            )
        elif rendering_mode == RenderingMode.HTML:
            return f"{self.indent_space}<h3>{make_markdown_one_line(self.rich_text, rendering_mode)}</h3>\n"
...
class Image(File, BaseBlock):
    ...
    def generate_tag(
        ...
        alt = (
            make_markdown_one_line(self.caption, rendering_mode) if self.caption else ""
        )
        url = ""
        filename = ""
        if self.type == FileType.FILE:
        ...
        if rendering_mode in [RenderingMode.MARKDOWN, RenderingMode.MARKDOWNX]:
            return f"{self.indent_space}![{alt}]({filepath})\n\n"
        elif rendering_mode == RenderingMode.HTML:
            return f"{self.indent_space}<img src='{filepath}' alt='{alt}'/>\n"
...
class NumberedListItem(BaseBlock):
    ...
    def generate_tag(
        ...
        if rendering_mode in [RenderingMode.MARKDOWN, RenderingMode.MARKDOWNX]:
            ...
            markdown += make_markdown_one_line(self.rich_text, rendering_mode)
            markdown += "\n\n"
            return markdown
        elif rendering_mode == RenderingMode.HTML:
            html = ""
