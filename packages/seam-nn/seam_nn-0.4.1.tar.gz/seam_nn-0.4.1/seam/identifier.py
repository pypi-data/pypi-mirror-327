...
class Identifier:
    ...
    def get_state_matrix(self, active_clusters, mode='binary'):
        ...
        mode : str
            ...
            'continuous': raw entropy values from revels matrix for each TFBS-cluster pair
        ...
        if mode == 'binary':
            ...
            for _, row in tfbs_df.iterrows():
                ...
                # Get original position indices
                original_indices = self.meta_explainer.cov_matrix.index.tolist()
                reordered_positions = [original_indices[self.meta_explainer.row_order.index(pos)] 
                                     for pos in positions]
                ...
                if reordered_positions:
                    ...
                    for cluster_idx in cluster_order:
                        ...
                        if self.meta_explainer.sort_method and self.meta_explainer.cluster_order is not None:
                            original_cluster_idx = self.meta_explainer.cluster_order[cluster_idx]
                        else:
                            original_cluster_idx = cluster_idx
                        
                        # Use revels for entropy calculation
                        cluster_entropy = self.meta_explainer.revels.iloc[original_cluster_idx, start:end + 1]
