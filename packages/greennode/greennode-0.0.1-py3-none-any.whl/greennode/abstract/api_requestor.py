...
import greennode
from greennode import error, utils
from greennode.constants import (
    BASE_URL,
    INITIAL_RETRY_DELAY,
...
from greennode.types import GreenNodeClient, GreenNodeRequest
...
def _make_session(max_retries: int | None = None) -> requests.Session:
    if greennode.requestssession:
        if isinstance(greennode.requestssession, requests.Session):
            return greennode.requestssession
        return greennode.requestssession()
    s = requests.Session()
    s.mount(
        "https://",
...
class APIRequestor:
    def __init__(self, client: GreenNodeClient):
    ...
    ) -> Tuple[Iterator[GreenNodeResponse], bool, str]:
        pass
    ...
    def request(
            ...
            remaining_retries: int | None = ...,
            request_timeout: float | Tuple[float, float] | None = ...,
    ) -> Tuple[GreenNodeResponse, bool, str]:
        pass
    ...
    def request(
            ...
            remaining_retries: int | None = ...,
            request_timeout: float | Tuple[float, float] | None = ...,
    ) -> Tuple[GreenNodeResponse | Iterator[GreenNodeResponse], bool, str]:
        pass
    ...
    def request(
        ...
        GreenNodeResponse | Iterator[GreenNodeResponse],
        bool,
        str | None,
    ]:
    ...
    async def arequest(
            ...
            stream: Literal[True],
            request_timeout: float | Tuple[float, float] | None = ...,
    ) -> Tuple[AsyncGenerator[GreenNodeResponse, None], bool, str]:
        pass
    ...
    async def arequest(
            ...
            stream: bool = ...,
            request_timeout: float | Tuple[float, float] | None = ...,
    ) -> Tuple[GreenNodeResponse | AsyncGenerator[GreenNodeResponse, None], bool, str]:
        pass
    ...
    async def arequest(
        ...
        if got_stream:
            ...
            async def wrap_resp() -> AsyncGenerator[GreenNodeResponse, None]:
    ...
    def handle_error_response(
        ...
        try:
            ...
            error_data = GreenNodeErrorResponse(**(error_resp))
    ...
    def _prepare_request_raw(
        ...
        if options.method.lower() == "get" or options.method.lower() == "delete":
            ...
            raise error.APIConnectionError(
                ...
                "GreenNode SDK. Please contact us by filling out https://greennode.ai/contact-us for "
    ...
    async def arequest_raw(
        ...
        try:
            ...
            raise error.APIConnectionError("Error communicating with GreenNode") from e

    def _interpret_response(
            self, result: requests.Response, stream: bool
    ) -> Tuple[GreenNodeResponse | Iterator[GreenNodeResponse], bool]:
    ...
    async def _interpret_async_response(
            ...
            tuple[AsyncGenerator[GreenNodeResponse, None], bool]
            | tuple[GreenNodeResponse, bool]
    ):
        """Returns the response(s) and a bool indicating whether it is a stream."""
        if stream and "text/event-stream" in result.headers.get("Content-Type", ""):
            return (
    ...
    def _interpret_response_line(
        ...
        if rcode == 204:
            return GreenNodeResponse({}, rheaders)
        ...
        resp = GreenNodeResponse(data, rheaders)
...
class AioHTTPSession(AsyncContextManager[aiohttp.ClientSession]):
    ...
    async def __aenter__(self) -> aiohttp.ClientSession:
        self._session = greennode.aiosession.get()
