...
class OverloopAuto(OverloopBase):
    ...
    def associate_call_to_contact(self, assoc, calls, contacts):
        ...
        if contact and call:
            ...
            assoc_entity = OverloopContactCreate.associate(
    ...
    def associate_contact_to_organization(self, assoc, contacts, organizations):
        ...
        organization = organizations.get(
            ...
            OverloopOrganizationCreate.get_existing(
                self.service, assoc.organization_domain
            ),
        )
        ...
        if contact and organization:
            ...
            if assoc_entity:
                actions.append(
                    f"Contact {contact.properties['email']} associated with organization {organization.properties['domain']}"
                )
                logger.debug(
                    f"Associated contact {contact.id} with organization {organization.id}"
                )
        else:
            if not contact:
                logger.debug(f"Contact not found for association: {assoc}")
    ...
    def associate_deal_to_organization(self, assoc, deals, organizations):
        ...
        if deal and organization:
            ...
            if assoc_entity:
                actions.append(
                    f"Deal {deal.properties['dealname']} associated with organization {organization.properties['domain']}"
                )
                logger.debug(
                    f"Associated deal {deal.id} with organization {organization.id}"
                )
        else:
            if not deal:
    ...
    def create_or_update(
        ...
        if existing_entity:
            if update_if_exists:
                ...
                logger.debug(
                    f"Entity already {root} (id={out_entity.id}) exists - updated"
                )
            else:
                out_entity = existing_entity
                logger.debug(
                    f"Entity already {root} (id={out_entity.id}) exists - skipping"
                )
            is_update = True
        else:
    ...
    def create_entities(
        ...
        for payload in payloads:
            ...
            url = getattr(
                out_entity, "url", f"https://app.overloop.com{root}/{out_entity.id}"
            )
            if is_update:
                actions.append(
                    f"{object_type_title} {unique_property_value} updated {url}"
                )
            else:
                actions.append(
                    f"{object_type_title} {unique_property_value} created {url}"
                )
        ...
        try:
            ...
            if actions:
                self.outputs["actions_report"] = (
                    "The following Overloop changes were made:\n" + "\n".join(actions)
                )
            else:
