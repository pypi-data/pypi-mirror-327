...
OPPORTUNITY_PROPERTY_DEFAULTS = {
    "CloseDate": lambda: (datetime.now(timezone.utc) + timedelta(days=30)).date()
}
...
def get_OpportunityStage_enum(service: Salesforce | None) -> Type[enum.Enum]:
    ...
    stages = service.query_all(
        "SELECT MasterLabel, ApiName FROM OpportunityStage ORDER BY SortOrder"
    )
