...
def __run(snake_installer, config, pdf, snakemake_other):
    """
    \b
    ...
    cmd_snakemake_base = f"snakemake -s {snake_installer.snakefile} --configfile {config} --workflow-profile {profile} {' '.join(rewrite_if_bind(snakemake_other))}"
    click.secho(f"\n    {cmd_snakemake_base}\n", fg='bright_blue')
