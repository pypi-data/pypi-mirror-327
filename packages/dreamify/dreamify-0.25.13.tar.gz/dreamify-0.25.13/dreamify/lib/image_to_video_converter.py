...
class ImageToVideoConverter:
    def __init__(self, dimensions, max_frames_to_sample):
        ...
        # Create a temporary directory to buffer frames
        if ImageToVideoConverter._temp_folder is None:
            ImageToVideoConverter._temp_folder = tempfile.mkdtemp()
            print(f"Temporary folder created at {ImageToVideoConverter._temp_folder}")
        else:
            print(f"Using existing temporary folder at {ImageToVideoConverter._temp_folder}")
        
        self.temp_folder = ImageToVideoConverter._temp_folder
    ...
    def to_video(self, output_path, duration, mirror_video, fps=60):
        ...
        frames = [
            ...
            for i in range(self.curr_frame_idx)
        ]
