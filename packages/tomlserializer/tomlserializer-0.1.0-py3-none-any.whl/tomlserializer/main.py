...
class TOMLSerializer:
    ...
    def _convert_from_toml(cls, value: Any, original_type: str) -> Any:
        ...
        try:
            ...
            raise TOMLDeserializationError(f"Failed to convert {value} from TOML: {str(e)}")
    ...
    def _extract_model_params(cls, model: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            raise TOMLSerializationError(f"Failed to extract model parameters: {str(e)}")
...
TOMLSerializer.register_type(
    ...
    lambda lst: [TOMLSerializer._convert_from_toml(item, type(item).__name__) for item in lst]
)

TOMLSerializer.register_type(
    tuple,
    lambda tpl: [TOMLSerializer._convert_to_toml_safe(item) for item in tpl],
    lambda tpl: tuple(TOMLSerializer._convert_from_toml(item, type(item).__name__) for item in tpl)
)

TOMLSerializer.register_type(
    set,
    lambda s: [TOMLSerializer._convert_to_toml_safe(item) for item in s],
    lambda s: set(TOMLSerializer._convert_from_toml(item, type(item).__name__) for item in s)
)
