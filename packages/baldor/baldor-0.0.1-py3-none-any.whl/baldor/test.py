...
def approximation_solutions(inputDirectory, verbose=False, log=False, count=False, bruteForce=False, approximation=False):
    ...
    if file_names:
        for file_name in file_names:
            ...
            app.approximation_solution(inputFile, verbose, log, count, bruteForce, approximation)


def main():
    
    # Define the parameters
    helper = argparse.ArgumentParser(prog="batch_solve", description="Estimating the Minimum Vertex Cover for all undirected graphs encoded in DIMACS format and stored in a directory.")
    ...
    approximation_solutions(args.inputDirectory, 
