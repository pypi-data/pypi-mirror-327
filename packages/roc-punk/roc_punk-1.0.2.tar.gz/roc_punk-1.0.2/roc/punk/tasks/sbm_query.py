...
class SBMQuery(Task):
    ...
    def run(self):
        ...
        logger.info(f"{len(sbms)} SBM type {self.sbm_type} for the period {period}")
        ...
        params["description"] = (
            f"{len(sbms)} SBM type {self.sbm_type} detections from {period}:\n\n"
        )
        params["description"] += (
            "@xbonnin @dberard @maksimov @jsoucek @oalexandrova\n\n"
        )
        for sbm in sbms:
            params["description"] += (
