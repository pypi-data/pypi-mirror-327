...
def get_ignored_patterns(directory: str) -> List[Tuple[str, str]]:
    ignored_patterns: List[Tuple[str, str]] = [
        ("poetry.lock", os.environ["CODEFORGEAI_USER_CODEBASE_DIR"]),
        ("package-lock.json", os.environ["CODEFORGEAI_USER_CODEBASE_DIR"]),
        (".DS_Store", os.environ["CODEFORGEAI_USER_CODEBASE_DIR"]),
        (".git", os.environ["CODEFORGEAI_USER_CODEBASE_DIR"]),
        *[(pattern, os.environ["CODEFORGEAI_USER_CODEBASE_DIR"])
          for pattern in os.environ["CODEFORGEAI_ADDITIONAL_EXCLUDE_DIRS"].split(',')
          if pattern]
    ]
