...
model = ChatBedrock(
    model_id=model_id,
    model_kwargs={"max_tokens": 4096, "temperature": 0.1, "top_k": 15},
...
def extract_codebase_and_attachments(x):
    codebase = get_combined_docs_from_files(x["config"].get("files", []))
...
agent = (
        {
            "codebase_and_attachments": lambda x: extract_codebase_and_attachments(x),
...
agent_executor = AgentExecutor(agent=agent,
                               tools=[],
                               verbose=True,
                               handle_parsing_errors=True).with_types(input_type=AgentInput)

agent_executor = agent_executor | (lambda x: x["output"])

if __name__ == "__main__":
    question = "How are you ?"
    print(agent_executor.invoke({"question": question, "chat_history": [], "attachments": []}))
