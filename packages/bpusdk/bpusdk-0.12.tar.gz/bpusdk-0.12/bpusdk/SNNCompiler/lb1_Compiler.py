...
from bpusdk.SNNCompiler.SMT_Base import SMTBase
    ...
    def __init__(self, config, neuron_num, netbase, network) -> None:
        ...
        if self.asic_flag:
            if self.config['cfg'] == 0:
                from .snn_compiler_96bit.func2smt_96bit_fp32 import SMT96bit
                # from .snn_compiler_96bit.func2smt_96bit_fp32_for_new_EINet import SMT96bit
                # from .snn_compiler_96bit.func2smt_96bit_fp32_for_new_EINet_v2 import SMT96bit
                
                
            elif self.config['cfg'] == 2:
                from .snn_compiler_96bit.func2smt_96bit_int16 import SMT96bit
            elif self.config['cfg'] == 3:
                from .snn_compiler_96bit.func2smt_96bit_int8 import SMT96bit
            self.smt_compiler = SMT96bit(self.netbase)        
        else:
            from .snn_compiler_32bit.fun2smt_32bit import SMT32bit
            ...
            for tile_id in range(self.chip_num):
                ...
                for npu_id in range(min(self.npu_num, 16)):
                    ...
                    for line in smt_result:
                        ...
                        num_parts = len(instr_bin) // 32
                        parts = [instr_bin[i * 32:(i * 32) + 32] for i in range(num_parts - 1, -1, -1)]
