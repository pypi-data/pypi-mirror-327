# This file is used to generate SMT constraints for the 96-bit ASIC at INT8 precision.
import math
import random
import copy
import numpy as np
...
class SMT96bit(SMTBase):
    ...
    def func_to_96bit_smt_cus(self, npu_id):
        ...
        if self.Ix_npu_random:
            if npu_id == 15:
                ...
                Ix_set = 1
                Ix_dec = np.uint32((Ix_set<<24) + (Ix_set<<16) + (Ix_set<<8) + (Ix_set<<0))
                ...
                smt_result[95:97] = post_func_stmts[0:]      ## NOTE, note where to replace the post_func_stmts
                return smt_result
    ...
    def func_to_96bit_smt_init(self,):
        ...
        trst_set, vrst = 5, 0
        trst = np.uint32((trst_set<<24) + (trst_set<<16) + (trst_set<<8) + (trst_set<<0))
        vrst = np.uint32((vrst<<24) + (vrst<<16) + (vrst<<8) + (vrst<<0))

        tau_set = 1
        tau_dec = np.uint32((tau_set<<24) + (tau_set<<16) + (tau_set<<8) + (tau_set<<0))
        ...
        Vth_set = 10
        Vth_dec = np.uint32((Vth_set<<24) + (Vth_set<<16) + (Vth_set<<8) + (Vth_set<<0))
        vth = IEEE754.ieee754_to_float(Vth_dec)
        vth = f'{vth:.60f}'

        W_decay1_set, E1_set = 1, 0
        W_decay1 = np.uint32((W_decay1_set<<24) + (W_decay1_set<<16) + (W_decay1_set<<8) + (W_decay1_set<<0))
        W_decay1 = IEEE754.ieee754_to_float(W_decay1)
        E1_dec   = np.uint32((E1_set<<24) + (E1_set<<16) + (E1_set<<8) + (E1_set<<0))
        E1       = IEEE754.ieee754_to_float(E1_dec)
        
        fac_dec = np.uint32((1 << 24) + (1 << 16) + (1 << 8) + (1 << 0))
        ...
        self.i_func = {
            "I0": lambda g0, V0: g0 * fac_dec ,
            "I1": lambda g1, V1: g1 * fac_dec ,
        }
