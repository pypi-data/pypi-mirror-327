...
class DemandsByDemander(Demands):
    ...
    def get_table_summary(self, obj, ar):
        ...
        items = [ar.obj2html(o, str(o.skill)) for o in sar]
...
class OffersByDemander(Offers):
    ...
    def get_table_summary(self, obj, ar):
        ...
        items = [ar.obj2html(o, str(o.end_user)) for o in sar]
...
class AssignableWorkersByTicket(Users):
    ...
    def get_request_queryset(self, ar):
        ...
        if topic:
            qs = qs.filter(skills_competence_set_by_user__topic=topic)
        cond = models.Q()
        for dem in rt.models.skills.Demand.objects.filter(demander=ticket):
            skills = dem.skill.get_parental_line()
            cond |= models.Q(skills_competence_set_by_user__faculty__in=skills)
        qs = qs.filter(cond)
...
if dd.is_installed('tickets'):
    ...
    class SuggestedTicketsByEndUser(Tickets):
        ...
        params_layout = """
        end_user feasable_by order #project state
