...
if with_suppliers:
    ...
    from .choicelists import OnboardingStates
    ...
    class SupplierChecker(Checker):
        ...
        def get_checkdata_problems(self, obj, fix=False):
            ...
            if obj.supplier_id:
                ...
                yield True, _("Must fill supplier from master data")
                if fix:
                ...
                yield True, _("Empty supplier row should get deleted")
                if fix:
                    obj.delete()
                return
    ...
    # SupplierChecker.activate() sets no_auto to True because we do not want
    # this checker to run automatically (i.e. during checkdata). It should run
    # only when called manually because it requires Ibanity credentials, which
    # are not available e.g. on GitLab.
