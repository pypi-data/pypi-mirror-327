# -*- coding: UTF-8 -*-
# Copyright 2012-2024 Rumma 6 Ko Ltd
...
def add_rules(rate, *vat_classes, **kwargs):
    for vat_class in vat_classes:
        for vat_regime, area in [('intracom', EU), ('cocontractor', NAT)]:
            add(vat_class, rate, area, 'purchases', vat_regime,
                CommonAccounts.vat_deductible, CommonAccounts.vat_returnable, **kwargs)
            add(vat_class, None, area, 'sales', vat_regime, **kwargs)

    for vat_class in vat_classes:
        for vat_regime, area in [('subject', NAT)]:
            add(vat_class, rate, NAT, 'purchases', vat_regime,
                CommonAccounts.vat_deductible, **kwargs)
    for vat_class in vat_classes:
        add(vat_class, rate, None, 'sales', None, CommonAccounts.vat_due, **kwargs)

add_rules("0.20", "services", "goods", "real_estate", "vehicles", end_date=i2d(20231231))
add_rules("0.09", "reduced", end_date=i2d(20241231))
add_rules("0.22", "services", "goods", "real_estate", "vehicles", start_date=i2d(20240101))
add_rules("0.13", "reduced", start_date=i2d(20250101))
...
mfld("1a",
     DC.credit,
     '10',
     "22% määraga maksustatavad müügid",
     vat_classes="goods services",
     vat_regimes="normal subject")
mfld("1b",
     DC.debit,
     '60',
     "22% määraga maksustatavad ostud liikmesriigi maksukohustuslaselt",
     vat_classes="goods services",
     vat_regimes="intracom")
sfld("1",
     DC.credit,
     None,
     "22% määraga maksustatavad toimingud ja tehingud",
     fieldnames="1a 1b")

mfld("2a",
     DC.credit,
     '10',
     "9% määraga maksustatavad müügid",
     vat_classes="reduced",
     vat_regimes="normal subject")
mfld("2b",
     DC.debit,
     '60',
     "9% määraga maksustatavad ostud liikmesriigi maksukohustuslaselt",
     vat_classes="reduced",
     vat_regimes="intracom")
sfld("2",
     DC.credit,
...
mfld("321",
     DC.credit,
     '10',
     "1) käibemaksutagastusega müük reisijale",
     vat_regimes="tax_free")

mfld("4",
     DC.credit,
     '40',
     "Käibemaks kokku (22% lahtrist 1 + 9% lahtrist 2)",
     is_payable=True)
mfld("41",
     DC.debit,
     '41',
...
mfld("53",
     DC.debit,
     '50', "3) ettevõtluses (100%) kasutatava sõiduauto soetamiselt ja sellise"
...
mfld("54",
     ...
     "tasutud või tasumisele kuuluv käibemaks",
     vat_classes="vehicles"
     )  #TODO: add an estonia-specific second vat category for vehicles?
...
mfld("61",
     DC.debit,
     '60',
     "1) kauba ühendusesisene soetamine",
     vat_regimes="intracom",
     vat_classes="goods")
...
mfld(
    ...
    vat_regimes="!intracom",
    vat_classes="!goods")  # not yet handled
...
sfld("13",
     DC.credit,
     None, "Tasumisele kuuluv(+) või enammakstud (-) käibemaks "
