...
def step_impl_assert_inbound_results(context, field_name, stream_name):
    ...
    # check if the sync_inbound_result is a list, if so, concatenate it
    if isinstance(context.sync_inbound_result[stream_name],list):
        sync_inbound_result_for_stream:pandas.DataFrame = pandas.concat(context.sync_inbound_result[stream_name])
    else:
        sync_inbound_result_for_stream:pandas.DataFrame = context.sync_inbound_result[stream_name]
    # sometimes we have a RECORD_DATA object instead of a RECORD_OBJECT object
    # if so, rename it
    if "RECORD_DATA" in sync_inbound_result_for_stream.columns:
        sync_inbound_result_for_stream = sync_inbound_result_for_stream.rename(columns={"RECORD_DATA":"RECORD_OBJECT"})
        # we also need to parse the RECORD_OBJECT column as json
        sync_inbound_result_for_stream["RECORD_OBJECT"] = sync_inbound_result_for_stream["RECORD_OBJECT"].apply(json.loads)
