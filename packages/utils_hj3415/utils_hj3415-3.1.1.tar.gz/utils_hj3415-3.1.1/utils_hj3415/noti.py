...
from utils_hj3415 import setup_logger, config
...
def mail_to(title: str, text: str, to_mail:str) -> bool:
    from_mail = config.get_env_variable("GMAIL_USER")
    app_pass = config.get_env_variable("GMAIL_APP_PASS")
...
def parse_telegram_tokens() -> dict:
    ...
    try:
        return json.loads(config.get_env_variable("TELEGRAM_BOT_TOKENS"))
...
def telegram_to(botname: str, text: str):
    """Telegram 메시지 전송"""
    chat_id = config.get_env_variable("TELEGRAM_CHAT_ID")
