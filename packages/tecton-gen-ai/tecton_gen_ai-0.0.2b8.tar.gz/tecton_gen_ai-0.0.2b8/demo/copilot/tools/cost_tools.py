import snowflake.connector
from typing import List, Optional, Dict, Any
from utils import _log_with_details, _err


class SnowflakeCostQueryTool:
    def __init__(self):
        self.connection = None

    def connect(self, user: str):
        self.connection = self.connection or snowflake.connector.connect(
            account="hza50686",
            user=user,
            warehouse="ANALYST_WAREHOUSE",
            database="DATAMART_PROD",
            schema="CONSUMPTION",
            authenticator='externalbrowser'  # Uses Okta login
        )

    def build_query(
        self,
        account_name: Optional[str],
        timestamp_start: Optional[str],
        timestamp_end: Optional[str],
        job_type: Optional[str],
        workspaces: Optional[List[str]],
        instance_type: Optional[str],
        fv_name: Optional[str]
    ) -> str:
        """
        Build the SQL query based on the input filters and groupings.
        """
        base_query = """
        SELECT
            ACCOUNT_NAME,
            UNIT,
            SUM(QUANTITY) AS QUANTITY,
            COALESCE(metadata:instance_type::string, 'Unknown') AS Instance_Type,
            COALESCE(metadata:job_type::string, 'Unknown') AS Job_Type,
            COALESCE(metadata:tecton_object_name::string, 'Unknown') AS FV_Name,
            COALESCE(metadata:workspace::string, 'Unknown') AS Workspace,
            TO_CHAR(TIMESTAMP, 'YYYY-MM-DD') AS TIMESTAMP,
            BASE_TECTON_CREDITS_PER_UNIT,
            SUM(QUANTITY) * BASE_TECTON_CREDITS_PER_UNIT AS TOTAL_TECTON_CREDITS
        FROM
            COMBINED_CONSUMPTION_METRICS_WITH_CREDITS_V2
        """

        # Add WHERE clause for filters
        where_clauses = []
        if account_name:
            where_clauses.append(f"ACCOUNT_NAME = '{account_name}'")
        if timestamp_start and timestamp_end:
            where_clauses.append(f"TIMESTAMP >= '{timestamp_start}' AND TIMESTAMP < '{timestamp_end}'")
        if job_type:
            where_clauses.append(f"COALESCE(metadata:job_type::string, 'Unknown') = '{job_type}'")
        if workspaces:  # Handle multiple workspaces
            workspace_conditions = ", ".join(f"'{workspace}'" for workspace in workspaces)
            where_clauses.append(f"COALESCE(metadata:workspace::string, 'Unknown') IN ({workspace_conditions})")
        if instance_type:
            where_clauses.append(f"COALESCE(metadata:instance_type::string, 'Unknown') = '{instance_type}'")
        if fv_name:
            where_clauses.append(f"COALESCE(metadata:tecton_object_name::string, 'Unknown') = '{fv_name}'")

        if where_clauses:
            base_query += "WHERE " + " AND ".join(where_clauses) + " "

        # Add GROUP BY clause
        group_by_fields = [
            "ACCOUNT_NAME",
            "Workspace",
            "UNIT",
            "FV_Name",
            "Job_Type",
            "Instance_Type",
            "TO_CHAR(TIMESTAMP, 'YYYY-MM-DD')",
            "BASE_TECTON_CREDITS_PER_UNIT"
        ]

        base_query += "GROUP BY " + ", ".join(group_by_fields) + " "

        # Add ORDER BY clause
        base_query += "ORDER BY TIMESTAMP, UNIT, QUANTITY DESC"

        return base_query

    def execute_query(self, query: str) -> List[Dict[str, Any]]:
        """
        Execute the SQL query and fetch results.
        """
        cursor = self.connection.cursor()
        try:
            _log_with_details(":question: Fetching cost information", f"```sql {query}")

            print(query)

            cursor.execute(query)
            columns = [col[0] for col in cursor.description]
            results = [dict(zip(columns, row)) for row in cursor.fetchall()]
            return results
        except Exception as e:
            print(f"Error executing query: {e}")
            return _err(f"{str(e)}")
        finally:
            cursor.close()

    def query_cost_data(
        self,
        user: str,
        account_name: str,
        timestamp_start: Optional[str] = None,
        timestamp_end: Optional[str] = None,
        job_type: Optional[str] = None,
        workspaces: Optional[List[str]] = None,
        instance_type: Optional[str] = None,
        fv_name: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        Query Tecton cost data

        Parameters:
        - user: The full email address of the currently logged in user
        - account_name: The name of the currently logged into Tecton account to filter by (e.g., "tecton-staging").
        - timestamp_start: Start of the timestamp range (e.g., "2024-05-01").
        - timestamp_end: End of the timestamp range
        - job_type: Filter by job type.(e.g., "BATCH_MATERIALIZATION").
        - workspaces: Filter by workspace names
        - instance_type: Filter by instance type
        - fv_name: Filter by feature view name

        Returns:
        - A list of dictionaries representing including the following fields:
            - UNIT
            - QUANTITY
            - Instance_Type
            - Job_Type
            - FV_Name
            - Workspace
            - TIMESTAMP (str that indicates the day)
            - TOTAL_TECTON_CREDITS
        """
        if not timestamp_start or not timestamp_end:
            return _err("Both timestamp_start and timestamp_end must be provided.")


        self.connect(user)

        query = self.build_query(
            account_name=account_name,
            timestamp_start=timestamp_start,
            timestamp_end=timestamp_end,
            job_type=job_type,
            workspaces=workspaces,
            instance_type=instance_type,
            fv_name=fv_name
        )
        return self.execute_query(query)
