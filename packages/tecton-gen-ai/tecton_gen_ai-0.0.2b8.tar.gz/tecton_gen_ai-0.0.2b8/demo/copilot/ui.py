...
from utils import _code, display, get_current_tecton_account_info, set_code_container

from tecton_gen_ai.api import Configs
from tecton_gen_ai.testing import set_dev_mode
from tecton_gen_ai.testing.log_utils import UILogHandler, set_ui_logger
from tecton_gen_ai.utils.log import cost_monitor
...
prompts.TECTON_ACCOUNT_INFO = (
    prompts.TECTON_ACCOUNT_INFO or get_current_tecton_account_info()
)
print(prompts.TECTON_ACCOUNT_INFO)
...
def _print_record(record) -> bool:
    ...
    if flag == "log":
        ...
        image_html = f'<img src="data:image/png;base64,{record.image_base64 }" width="800"/>'
...
def _display(markdown: str, show_flowchart: bool):
    def _show_flowchart_code(code: str):
        st.code(code, language="json")

    if show_flowchart:
        display(markdown, st.markdown, _display_flowchart)
    else:
        display(markdown, st.markdown, _show_flowchart_code)
...
#         width: 120px !important;  /* Fixed width for the first column */
#         flex: unset !important;
#     }
#     [data-testid="column"]:nth-child(2) {
#         flex: 1;  /* Allow the second column to take the remaining space */
#     }
#     </style>
# """, unsafe_allow_html=True)

# col1, col2 = st.columns([1, 10])  # You can keep the relative sizes for layout
# with col1:
#     st.image(MELVIN_PATH, width=120)
# with col2:
#     st.markdown('<h1 style="margin-top: 24px;">Melvin - Your Tecton Copilot</h1>', unsafe_allow_html=True)
    ...
    with container:
        ...
        for i, message in enumerate(st.session_state.messages):
            ...
            with st.chat_message(message["role"], avatar=avatar):
                _display(message["content"], show_flowchart=True)
                if "log" in message:
                    is_last = i == last
...
def on_submit():
    ...
    with container:
        ...
        with st.chat_message("user"):
            _display(prompt, show_flowchart=False)
        ...
        with ct:
            ...
            with empty:
                _display(response, show_flowchart=False)
