...
_DeviceClassT = typing.TypeVar("_DeviceClassT")
...
class SystemInfo(abc.ABC):
    ...
    device_models: list[str] | dict[int | str, list[str]] = []
    system_versions: list[str] = []

    @classmethod
    def random_device(cls: type[SystemInfo], unique_id: str = None) -> DeviceInfo:
        hash_id = cls.strtohashid(unique_id)
        return cls._random_device(hash_id)

    @classmethod
    def _random_device(cls, hash_id: int):
        cls.__gen__()
        return cls.hashtovalue(hash_id, cls.device_list)
    ...
    @classmethod
    def strtohashid(cls, unique_id: str = None):
    ...
    @classmethod
    def hashtovalue(cls, hash_id: int, values: list[_DeviceClassT]) -> _DeviceClassT:
        return values[hash_id % len(values)]

    @classmethod
    def clean_and_simplify(cls, text: str) -> str:
...
class GeneralDesktopDevice(SystemInfo):
    ...
    # just like tdesktop does in lib_base
    # https://github.com/desktop-app/lib_base/blob/master/base/platform/win/base_info_win.cpp#L173
    #
    # Feel free to use
    #
    # Sources: https://answers.microsoft.com/, https://www.techsupportforum.com/ and https://www.bleepingcomputer.com/
    # TODO: Crawl more device models
...
class WindowsDevice(GeneralDesktopDevice):
    system_versions: list[str] = ["Windows 11", "Windows 10", "Windows 8", "Windows 8.1", "Windows 7"]

    @classmethod
    def __gen__(cls: type[WindowsDevice]):
        if len(cls.device_list) == 0:
            results: list[DeviceInfo] = []

            for model in cls.device_models:
                model = cls.clean_and_simplify(model.replace("_", ""))

                for version in cls.system_versions:
                    results.append(DeviceInfo(model, version))

            cls.device_list = results


class LinuxDevice(GeneralDesktopDevice):
    @classmethod
    def __gen__(cls: type[LinuxDevice]):
        if len(cls.system_versions) == 0:
            ...
            def _getitem(group: list[list[str]], prefix: str = "") -> list[str]:
                ...
                for item in group[0]:
                    results.extend(_getitem(group[1:], prefix + item))

                return results

            libc_full_names = _getitem([libc_names, libc_vers], "")
            cls.system_versions = _getitem([environments, wayland, libc_full_names], "Linux")
        ...
        if len(cls.device_list) == 0:
            ...
            for version in cls.system_versions:
                for model in cls.device_models:
                    devices.append(DeviceInfo(model, version))

            cls.device_list = devices


class AndroidDevice(SystemInfo):
    # TODO: Add new android device models
    device_models = [
        "Samsung GT-I5510M",
        "Samsung GT-I5800L",
        "Samsung SCH-I559",
    ...
    @classmethod
    def __gen__(cls: type[AndroidDevice]):
        if len(cls.device_list) == 0:
            results: list[DeviceInfo] = []
...
class MacOSDevice(GeneralDesktopDevice):
    ...
    # TODO: Add new Mac device models
    device_models: list[str] = [
        "MacBookPro16,4",
        "MacBookPro16,3",
        "MacBookPro16,2",
    ...
    @classmethod
    def __gen__(cls: type[MacOSDevice]):
        if len(cls.device_list) == 0:
            # https://github.com/desktop-app/lib_base/blob/master/base/platform/mac/base_info_mac.mm#L42
            def from_identifier(model_identifier: str):
                words = []
                word = ""

                for ch in model_identifier:
                    if not ch.isalpha():
                        continue
                    if ch.isupper():
                        if word != "":
            ...
            new_devices_models = []
            for model in cls.device_models:
                model = cls.clean_and_simplify(from_identifier(model))
                if model not in new_devices_models:
...
class IOSDevice(SystemInfo):
    ...
    @classmethod
    def __gen__(cls: type[IOSDevice]):
        if len(cls.device_list) == 0:
            results: list[DeviceInfo] = []
...
class APIData(abc.ABC):
    api_id: int | None = None  # type: ignore
    api_hash: str | None = None  # type: ignore
    device_model: str | None = None  # type: ignore
    system_version: str | None = None  # type: ignore
    app_version: str | None = None  # type: ignore
    lang_code: str | None = None  # type: ignore
    system_lang_code: str | None = None  # type: ignore
    lang_pack: str | None = None  # type: ignore
    ...
    def __init__(
        ...
        self.api_id = api_id if api_id else self.__class__.api_id
        self.api_hash = api_hash if api_hash else self.__class__.api_hash
        self.device_model = device_model if device_model else self.__class__.device_model
        self.system_version = system_version if system_version else self.__class__.system_version
        self.app_version = app_version if app_version else self.__class__.app_version
        self.system_lang_code = system_lang_code if system_lang_code else self.__class__.system_lang_code
        self.lang_pack = lang_pack if lang_pack else self.__class__.lang_pack
        self.lang_code = lang_code if lang_code else self.__class__.lang_code

        if self.device_model is None:
            system = platform.uname()

            if system.machine.lower() in ("x86_64", "amd64", "arm64"):
                self.device_model = "PC 64bit"
            elif system.machine in ("i386", "i686", "x86"):
                self.device_model = "PC 32bit"
            else:
                self.device_model = system.machine.lower()

    @classmethod
    def generate(
            cls: type[_T],
            unique_id: str = None,
            system: typing.Literal["windows", "linux", "macos"] | None = None
    ) -> _T:
        if issubclass(cls, API.TelegramAndroid):
            device_info = AndroidDevice.random_device(unique_id)
        elif issubclass(cls, API.TelegramIOS):
            device_info = IOSDevice.random_device(unique_id)
        elif issubclass(cls, API.TelegramMacOS):
            device_info = MacOSDevice.random_device(unique_id)
        elif issubclass(cls, API.TelegramDesktop):
            if system is None:
                system = "windows"
            else:
                system = system.lower()
            ...
            if system == "windows":
                device_info = WindowsDevice.random_device(unique_id)
            elif system == "macos":
                device_info = MacOSDevice.random_device(unique_id)
            else:
                device_info = LinuxDevice.random_device(unique_id)
        else:
            raise NotImplementedError(f"{cls.__name__} device not supported for randomize yet")

        return cls(device_model=device_info.model, system_version=device_info.version)
...
class API(abc.ABC):
    ...
    class TelegramAndroidX(TelegramAndroid):
        """
    ...
    class TelegramPlus(TelegramAndroid):
        api_id = 16623
        api_hash = "8c9dbfe58437d1739540f5d53c72ae4b"


class MetadataGenerator:
    def __init__(self, telegram_platform: TelegramPlatform, session_name: str):
        self.platform = telegram_platform
        self.session_name = session_name

    def generate(self) -> SessionMetadata:
        if self.platform == TelegramPlatform.IOS:
            api_data = API.TelegramIOS.generate(unique_id=self.session_name)
        elif self.platform == TelegramPlatform.MACOS:
            api_data = API.TelegramMacOS.generate(unique_id=self.session_name)
        elif self.platform == TelegramPlatform.ANDROID:
            api_data = API.TelegramAndroid.generate(unique_id=self.session_name)
        elif self.platform == TelegramPlatform.DESKTOP_MACOS:
            api_data = API.TelegramDesktop.generate(unique_id=self.session_name, system="macos")
        elif self.platform == TelegramPlatform.DESKTOP_WINDOWS:
            api_data = API.TelegramDesktop.generate(unique_id=self.session_name, system="windows")
        elif self.platform == TelegramPlatform.DESKTOP_LINUX:
            api_data = API.TelegramDesktop.generate(unique_id=self.session_name, system="linux")
        elif self.platform == TelegramPlatform.WEB:
            api_data = API.TelegramWebZ.generate(unique_id=self.session_name)
        else:
            api_data = API.TelegramDesktop.generate(unique_id=self.session_name, system="windows")

        return SessionMetadata(
            phone_number=self.session_name,
            ...
            lang_code=api_data.lang_code or 'en',
            system_lang_code=api_data.system_lang_code or 'en-US',
            lang_pack=api_data.lang_pack or '',
        )
