...
class SQLValidator(BaseSQLValidator):
    ...
    def __call__(self, prompt_obj, debug=False):
        ...
        if self.sql_key not in prompt_obj.data:
            raise ValueError(f"{self.sql_key} not found in prompt object data")

        query = prompt_obj.data[self.sql_key]
        ...
        try:
            ...
            prompt_obj.response = {self.is_valid_field: True, "error": None}

        except Exception as e:
            # If there's an error, query is invalid
            prompt_obj.response = {self.is_valid_field: False, "error": str(e)}
