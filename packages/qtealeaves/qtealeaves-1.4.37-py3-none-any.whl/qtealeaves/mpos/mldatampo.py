...
class MLDataMPO(_AbstractEffectiveOperators):
    ...
    def __init__(self, data_samples, labels, batch_size, tensor_backend):
        ...
        for ii in range(num_samples):
            for jj in range(num_sites):
                samples[jj].set_subtensor_entry(
                    [0, 0, 0, 0, ii],
                    [1, 1, 2, 1, ii + 1],
                    data_samples[ii][jj].reshape([1, 1, local_dim, 1, 1]),
                )
    ...
    def from_matrix(
        ...
        feature_map_0 : callable for state |0>
            Feature map function to map a data point into the |0>-state of the qubit.

        feature_map_1 : callable for state |1>
            Feature map function to map a data point into the |1>-state of the qubit.
        """
        local_dim = 2  # see two feature map functions
        num_samples = data_samples.shape[0]
        num_sites = data_samples.shape[1]
        ...
        for ii in range(num_sites):
            ...
            sub_tensor_0 = tensor.from_elem_array(
                feature_map_0(data_samples[:, ii]).reshape(1, 1, 1, 1, num_samples),
                dtype=tensor_backend.dtype,
                device=tensor_backend.memory_device,
            )
            sub_tensor_1 = tensor.from_elem_array(
                feature_map_1(data_samples[:, ii]).reshape(1, 1, 1, 1, num_samples),
                dtype=tensor_backend.dtype,
                device=tensor_backend.memory_device,
            )

            tensor.set_subtensor_entry(
                [0, 0, 0, 0, 0],
                [1, 1, 1, 1, num_samples],
                sub_tensor_0,
            )
    ...
    def setup_as_eff_ops(self, tensor_network, measurement_mode=False):
        ...
        if tensor_network.extension in ["mps"]:
            ...
            for ii in range(self.batch_size):
                eff_op.set_subtensor_entry(
                    [0, 0, 0, 0, ii], [1, chi_l, chi_l, 1, ii + 1], eye
                )
