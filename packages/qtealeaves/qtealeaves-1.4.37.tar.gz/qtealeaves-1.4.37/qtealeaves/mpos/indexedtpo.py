...
from .densempos import DenseMPO, DenseMPOList, MPOSite
...
class ITPOTerm(_ParameterizedClass):
    ...
    def stream(self, disable_streams=False):
        ...
        """
        for _, elem in self._tensors.items():
            return elem.stream(disable_streams=disable_streams)

        if self._local is not None:
            return self._local.stream(disable_streams=disable_streams)
    ...
    * iucTPO : iTPO with compression and smart update for time-dependent parameters.
    ...
    def contract_tensor_lists(
        ...
        **Arguments**
        ...
        custom_function : Callable or None
            A callable which can modify the result. Input is a single tensor.
            Default None does not modify the result.
        """
    ...
    def get_local_kraus_operators(self, dt):
        ...
        sqrt, abs = self.site_terms[0]._local_ops[0].get_attr("sqrt", "abs")
        ...
        for site_idx, siteterm in enumerate(self.site_terms):
            ...
            for indx, lam_indx in enumerate(lam.elem):
                if abs(lam_indx) > lam.dtype_eps:
                    inds.append(indx)

            lam._elem = sqrt(lam.elem)
            lam._elem = lam.diag()
            ...
            kraus_tensor = kraus_tensor.subtensor_along_link_inds(link=0, inds=inds)
