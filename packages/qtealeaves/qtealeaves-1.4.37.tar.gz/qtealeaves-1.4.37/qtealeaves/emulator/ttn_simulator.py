...
from qtealeaves.tooling.hilbert_curvature import QTeaLeavesError, map_selector
...
class TTN(_AbstractTN):
    ...
    def get_substate(self, first_site, last_site, truncate=True):
        ...
        last_site : int
            ...
            Python indexing assumed, i.e. counting starts from 0.
        truncate : Bool
            If False, tensors are returned as is, i.e. TTO. If True, the edges of will be
            truncated to dummy links, i.e. TTN. Default to True.
            Default to False.
        """
        raise NotImplementedError(
            "Not yet implemented for TTNs, but implementation is possible."
        )
    ...
    def product_state_from_local_states(
        ...
        for ii in range(num_sites_tree // 2):
            ...
            if i1 < num_sites:
                ...
                mat1 = tensor_backend.from_elem_array(mat[i1])
            else:
                local_dim1 = 1
                mat1 = tensor_backend([1, 1], ctrl="O").reshape([1, 1])
            if i2 < num_sites:
                local_dim2 = local_dim[i2]
                mat2 = tensor_backend.from_elem_array(mat[i2])
            else:
                local_dim2 = 1
    ...
    def kron(self, other, inplace=False, fill_identity=False):
        """
        ...
        if fill_identity:
            ...
            symm_tens, topright = topright.split_qr(
                [2],
                [0, 1],
                perm_left=[1, 0],
                perm_right=[1, 2, 0],
        ...
        for ii in range(self.num_layers - 1, 0, -1):
            ...
            for jj in range(len(self[ii])):
                layer_tensors.append(self[ii][jj])
            for jj in range(len(other[ii])):
                layer_tensors.append(other[ii][jj])
        ...
        for layer in self.layers[::-1]:
            ...
            for tidx in range(len(layer) - 1, -1, -1):
                ...
                if tidx > 0:
                    ...
                    # move the next tensor (tidx-1) using a stream while building the
                    # operators for tidx
                    self.move_pos(
                        (lidx, tidx - 1),
                        device=self._tensor_backend.computational_device,
    ...
    def sum_approximate(
        ...
        **Returns**
        ...
        for ii in range(max_iterations):
            ...
            for ll, layer in enumerate(result):
                for tens, _ in enumerate(layer):
                    ...
                    for ii, local_projector in enumerate(local_projectors):
                        prefactor_ii = np.conj(sum_amplitudes[ii])
            ...
            # We can step when all differences are smaller than the dif_goal.
    ...
    def get_projector_function(self, pos, pos_links):
        ...
        **Returns**
        ...
        # The function does not modify the optional argument.
        # pylint: disable-next=dangerous-default-value
        def project_out_projectors(
            tensor, projectors=self.eff_proj, pos=pos, pos_links=pos_links
        ):
            """
            ...
            **Returns**
            ...
            for projector in projectors:
                ...
                abs_overlaps.append(abs(overlap))
                local_projectors.append(local_projector)

            # and now orthogonalize starting with the largest overlap
            argsort = tensor.get_attr("argsort")
            for ii in argsort(abs_overlaps)[::-1]:
                ...
                # It seems to even reduce performance. Luka Oct 2024

            return tensor

        return project_out_projectors
            ...
            for idx, singvals in enumerate(layer.singvals):
                ...
                if singvals is None:
                    ...
                    if (
                        ...
                        singvals = np.array(flatten(singvals))
