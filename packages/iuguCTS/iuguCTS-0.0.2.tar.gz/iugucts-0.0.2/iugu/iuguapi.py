...
class IuguApi(object):
    ...
    def base_request(self, url, method, data={}):
        try:
            ...
            raise Exception(f"JSON decode error. Response: {response.text}.")
        except requests.exceptions.Timeout:
            raise TimeoutError("Request timeout.")
        except requests.exceptions.ConnectionError:
            raise ConnectionError("Request connection error.")
        except requests.exceptions.HTTPError as e:
            raise RuntimeError(f"Request error HTTP: {e.response.status_code}.")
        except requests.exceptions.RequestException as e:
            raise RuntimeError(f"Request error inesperado: {e}.")
