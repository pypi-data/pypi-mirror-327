...
class ArnelifyServer:
  def __init__(self, opts):
    ...
    if platform.architecture()[0] != '64bit':
      print("[Arnelify Server FFI]: CPU platform isn't 64bit.")
      exit(1)
    
    if platform.machine() == 'aarch64':
      self.lib = self.ffi.dlopen('./arnelify_server/bin/arnelify_server_arm64.so')

    elif platform.machine() == 'x86_64':
      self.lib = self.ffi.dlopen('./arnelify_server/bin/arnelify_server_amd64.so')
    else:
      print("[Arnelify Server FFI]: CPU platform isn't supported.")
      exit(1)
