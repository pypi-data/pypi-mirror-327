...
class IBMProvider(BaseProvider):
    """Provider implementation for IBM Cloud."""
    
    def __init__(self):
        """
        Initialize IBM Cloud provider with region data.
        Loads region information from the ibm_regions.json mapping file.
        """
        base_path = os.path.dirname(os.path.abspath(__file__))
        data_path = os.path.join(base_path, "..", "mappings", "ibm_regions.json")
        with open(data_path, "r", encoding="utf-8") as f:
            self.region_data = json.load(f)

    def get_region_info(self, region: str) -> dict:
        """
        Get information about an IBM Cloud region.

        Args:
            region (str): IBM Cloud region code (e.g., 'us-south', 'eu-gb')

        Returns:
            dict: Region information containing:
                - location (str): Human-readable location (e.g., "Dallas")
