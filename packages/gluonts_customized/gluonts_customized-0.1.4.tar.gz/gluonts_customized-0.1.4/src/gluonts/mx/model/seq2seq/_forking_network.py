...
from src.gluonts.mx import Tensor
from src.gluonts.mx.block.decoder import Seq2SeqDecoder
from src.gluonts.mx.block.enc2dec import Seq2SeqEnc2Dec
...
from src.gluonts.mx.block.scaler import MeanScaler, NOPScaler
from src.gluonts.mx.distribution import DistributionOutput
from src.gluonts.mx.util import weighted_average
...
class ForkingSeq2SeqNetworkBase(gluon.HybridBlock):
    ...
    def get_decoder_network_output(
        ...
        # enc_output_dynamic shape:
        # (batch_size, encoder_length, channels_seq[-1] + 1)
        ...
        # where we we only need to pass the encoder output for the last time
        # step
        dec_output = self.decoder(dec_input_static, dec_input_dynamic)

        # the output shape should be: (batch_size, num_forking, dec_len,
        # decoder_mlp_dim_seq[0])
...
class ForkingSeq2SeqTrainingNetwork(ForkingSeq2SeqNetworkBase):
    def hybrid_forward(
        ...
        """
        # shape:
        # (batch_size, num_forking, decoder_length,decoder_mlp_dim_seq[0])
...
class ForkingSeq2SeqPredictionNetwork(ForkingSeq2SeqNetworkBase):
    def hybrid_forward(
        ...
        # shape: (batch_size = num_test_ts, decoder_length = prediction_length,
        # num_quantiles)
