...
def create_image_flux(messages, **kwargs):
    ...
    flux.txt_to_img(
        image_prompt,
        width=config.image_width if hasattr(config, "local_image_width") and config.image_width else 1024,
        height=config.image_height if hasattr(config, "local_image_height") and config.image_height else 1024,
