...
class Download():
    ...
    def er(self, bookmark_name, save_path, direct_export=False, filter_name='', stop_date='', start_date='', filters=10, extension='csv'):
        ...
        if direct_export:
            ...
            while attempts < 5:
                try:
                    ...
                    print('Unable to find {} in list'.format(bookmark_name))
                    attempts += 1
                    elements = self.driver.find_elements(By.CLASS_NAME , 'CatalogActionLink')
                    element_text = [elem.text for elem in elements]
                    time.sleep(1)
            ...
            for element in self.driver.find_elements(By.CLASS_NAME, 'contextMenuOptionText'):       # Click Data
                if element.text == 'Data':
                    element.click()
                    break
            self.driver.find_element(By.ID, 'menuOptionItem_CSV').click()                           # Click CSV
            while True:     # Wait for the download confirmation popup to show up
                ...
                if self.driver.find_element(By.CLASS_NAME, 'dialogTitle').text == 'Confirmation':
                    ...
                    break
        else:       # This block hits the edit button and goes to criteria
            while True:
                try:
            ...
            self.edit_filter(filter_name, stop_date, start_date, filters)
