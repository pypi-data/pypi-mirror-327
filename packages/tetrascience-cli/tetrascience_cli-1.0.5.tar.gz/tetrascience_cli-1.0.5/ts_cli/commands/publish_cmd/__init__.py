...
def publish_cmd_args(parser: argparse.ArgumentParser):
    ...
    artifact_config.add_argument(
        "--type",
        type=str,
        choices=["ids", "protocol", "task-script", "tetraflow"],
        help="artifact type",
    )
    artifact_config.add_argument(
        "--namespace",
...
def monitor_task_script_build(ts_api: TsApi, build_id: str):
    """
    Print task script build logs to the console
    :param ts_api:
    :param build_id:
    :return:
    """
    ...
    while True:
        ...
        logs_resp = ts_api.get_task_script_build_logs(
            build_id, {"nextToken": prev_next}
        )
        prev_next = logs_resp.get("nextToken", None)
...
def publish(args):
    ...
    upload_size_checker = UploadValidator(upload_content=zip_bytes, exiting=True)
    ...
    if args.dry_run is not True:
        ...
        if artifact_config.type == "task-script":
            ...
            if build_id:
                monitor_task_script_build(ts_api, build_id)
            else:
