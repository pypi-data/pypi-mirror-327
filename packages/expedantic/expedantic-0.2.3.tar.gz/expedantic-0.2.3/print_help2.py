...
def create_field_tree(fields: list[FieldInfo]) -> Tree:
    ...
    def format_field(field: FieldInfo) -> str:
        ...
        required_style = "bold magenta" if field.is_required else "magenta"
        type_style = "bold green" if field.is_required else "green"

        parts = [
            (f"--{field.full_path}", required_style),
            (f"({field.type_str})", type_style),
        ]

        if field.default != "Required":
            parts.append((f"[default: {field.default}]", "yellow"))

        if field.description:
            parts.append((f"- {field.description}", "white"))

        return " ".join(f"[{style}]{text}[/]" for text, style in parts)
