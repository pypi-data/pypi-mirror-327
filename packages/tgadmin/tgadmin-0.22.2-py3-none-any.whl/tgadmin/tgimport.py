...
def aggregation_import(tgcrud: TextgridCrud, sid: str, project_id: str, filenames: List[str], threaded: bool, ignore_warnings: bool):
    """
    ...
    for filename in filenames:
        ...
        if is_portalconfig(meta):
            ...
            portalconfig_file = filename
        elif is_readme(meta):
            upload_file(tgcrud, sid, project_id, PurePath(filename), imex_map, ignore_warnings)
        else:
            tguri = handle_aggregation_upload(tgcrud, sid, project_id, filename, meta, imex_map, threaded, ignore_warnings)
            aggregation_filename = filename

    # finally rewrite and upload portalconfig
    if portalconfig_file != "":
         handle_portalconfig_upload(tgcrud, sid, project_id, portalconfig_file, imex_map, ignore_warnings)
...
def handle_portalconfig_upload(tgcrud, sid, project_id, filename, imex_map, ignore_warnings):
    ...
    meta = metafile_to_object(filename, referenced_in=filename)
    content = rewrite_portalconfig_file(filename, imex_map)
    tguri = upload_modified(tgcrud, sid, project_id, content, meta, ignore_warnings, filename, default_namespace=PC_NAMESPACES["pc"])
...
def upload_modified(tgcrud, sid, project_id: str, etree_data, metadata, ignore_warnings, filename="", default_namespace = None) -> str:
    ...
    data_str = ET.tostring(etree_data, encoding="utf8", method="xml", default_namespace=default_namespace)
