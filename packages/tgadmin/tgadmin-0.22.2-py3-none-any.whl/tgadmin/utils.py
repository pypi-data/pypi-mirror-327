...
def imex_to_path_dict(imex_file_path) -> dict:
    """parse textgrid_uris and paths from imex file. return a dict with purepath as key, tguri as value"""
...
def rewrite_portalconfig_string(portalconfig, avatar_location, xslt_location = ""):
    ...
    the_data = ET.tostring(
        the_dataXML, encoding="utf8", method="xml", default_namespace=PC_NAMESPACES["pc"]
    )
    return the_data


def rewrite_portalconfig_file(portalconfig_file, imex_map):
    ...
    avatar_location.text = imex_map[PurePath(data_path / avatar_location.text)]
    # optionally modify xslt element
    xslt_tag = the_dataXML.find("pc:xslt", PC_NAMESPACES)
    if xslt_tag:
        xslt_tag.text = imex_map[PurePath(data_path / xslt_tag.text)]
