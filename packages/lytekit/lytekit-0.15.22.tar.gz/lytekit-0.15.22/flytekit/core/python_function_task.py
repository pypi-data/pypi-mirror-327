...
class PythonFunctionTask(PythonAutoContainerTask[T]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        def _hash_function(task_func: Callable):
            func_str = inspect.getsource(task_func)
            module = ast.parse(func_str)

            if not (len(module.body) == 1 and isinstance(module.body[0], ast.FunctionDef)):
                raise ValueError(f"Unable to hash callable {self._task_function}")

            # Function name should not effect hash
            func_node = module.body[0]
            func_node.name = ""

            # Strip docstrings from ast
        ...
        if self._metadata.cache and not self._metadata.cache_version:
            self._metadata.cache_version = _hash_function(task_function)
