...
def get_pkg_version():
    ...
    return metadata.version("lytekit")
...
class FlyteScopedException(Exception):
    ...
    def verbose_message(self):
        ...
        lines = "".join(_format_tb(top_tb, limit=limit)).replace("  ", "    ")
        traceback_str = indent(lines, "    " * 3)
...
{traceback_str}
            ...
            Metadata:
                FlyteKit Version: {get_pkg_version()}
                Python Version: {py_version}
                Platform: {platform.platform()}
                OS: {os.name}, {platform.version()}
        """)
    ...
    try:
        ...
        if _is_base_context():
            ...
            try:
                ...
                raise FlyteScopedSystemException(
