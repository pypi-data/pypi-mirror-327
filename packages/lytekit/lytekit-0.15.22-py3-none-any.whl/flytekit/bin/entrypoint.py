...
def _dispatch_execute(
    ...
    # Handle user-scoped errors
        ...
        traceback_path = pathlib.Path(
            os.getenv("latch_traceback_path", "/opt/latch/traceback.txt")
        )
        traceback_path.parent.mkdir(exist_ok=True)
        traceback_path.write_text(e.verbose_message)

        with traceback_path.open("w") as f:
            _traceback.print_exception(e, file=f)
