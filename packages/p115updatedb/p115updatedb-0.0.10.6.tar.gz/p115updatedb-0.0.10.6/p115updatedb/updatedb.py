...
from .query import (
    ...
    iter_id_to_parent_id, select_mtime_groups, iter_descendant_ids, 
)
...
def diff_dir(
    ...
    **request_kwargs, 
    ...
    if refresh or not ((dirlen := get_dir_count(con, id)) and dirlen["tree_file_count"]):
        future1 = run_as_thread(lambda: set(iter_descendant_ids(con, id, tree=tree)))
...
def updatedb_one(
    ...
    **request_kwargs, 
    ...
    upsert_items(cur, to_upsert, extras={"_triggered": 0}, commit=True)
    kill_items(cur, to_remove, commit=True)
