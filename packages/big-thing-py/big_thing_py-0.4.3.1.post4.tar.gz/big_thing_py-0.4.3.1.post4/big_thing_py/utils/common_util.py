...
def normalize_bound(type: MXType, bound: Tuple[Any, Any] = None) -> Tuple[Any, Any]:
    if type in [MXType.STRING, MXType.BINARY]:
        if bound:
            cprint('`bound` parameter will be ignored because type is STRING or BINARY. set bound to -1, -1', 'yellow')

        return -1, -1
    elif type == MXType.BOOL:
        if bound:
            cprint('`bound` parameter will be ignored because type is BOOL. set bound to 0, 1', 'yellow')
