...
from gias3.fieldwork.field.tools import triangulate
...
class GeometricField:
    ...
    def __init__(self, name, dimensions, ensemble_field_function=None, field_dimensions=None, field_basis=None):
        ...
        self.ensemble_field_function_old = None
        ...
        self.triangulator = triangulate.MeshTriangulator(self.ensemble_field_function, self.field_parameters, False)
    ...
    def add_geometric_spline(self, points, name, smoothing=3.0, order=3):
        ...
        self.splines[name] = Spline3dParametric(coords, order, smoothing)
    ...
    # ==================================================================#
    def invertSurfaceMesh(self, node_dist_tol=1e-3):
        """
        ...
        for elemNumber in list(M.keys()):
            ...
            self.add_element_with_parameters(elem, elemParams, tol=node_dist_tol)
    ...
    def evaluate_geometric_field_at_element_points_3(self, EP):
        ...
        [(0, [0.5, 0.1]), (1, [0.1, 0.1]),]
        """
        evaluator = makeGeometricFieldEvaluatorSparse(self, [1, 1], mat_points=EP)
    ...
    # ==================================================================#
    def find_closest_material_points(self, data_points, init_gd=None, verbose=False):
        """
        returns 
        closestMPs = [ (elem, xi),... ]
        ...
        if verbose:
            log.debug('searching for closest material point for %i points' % (len(data_points)))

        if init_gd is None:
            init_gd = numpy.ones(self.ensemble_field_function.dimensions, dtype=int) * 40
        ...
        initXi = [e.generate_eval_grid(init_gd) for e in elements]
        ...
        initClosestInd = initEPTree.query(list(data_points))[1]
        closestMPs = []
        ...
        closestPoints = []
        distances = []
        for pi, p in enumerate(data_points):
    ...
    def calc_CoM_2D(self, d, elem=None):
        ...
        aT, a, T, ep = self.calc_surface_area(d, element=elem, full_output=True)
    ...
    # ==================================================================#
    def calc_surface_area(self, d, element=None, full_output=False):
    ...
    # ==================================================================#
    def _draw_surface(self, density, scalar=None, figure=None, name=None, lim=None):
        """ using triangulate to draw the interpolated surface
        """
        lim = [None, None] if lim is None else lim
    ...
    # ==================================================================#
    def _draw_surface_curvature_binned(self, field_eval_density, curvature, name, bins, c_min, c_max, fig=None):
    ...
    # ==================================================================#
    def makeLineElementsFromPoints(self, points, nodes_per_elem, elem_basis_map):
        ...
        elemTypes = [list(elem_basis_map.keys())[0]] * nElems
        x = 0
        for nE in range(nElems):
            e = element_types.create_element(elemTypes[nE])
            lineGF.add_element_with_parameters(e, params[:, x:x + nodes_per_elem, :], tol=1e-3)
            x += nodes_per_elem - 1

        return lineGF

    # ==================================================================#
    def makeLineElementsFromPointSets(self, point_sets, elem_types, elem_basis_map):
        ...
        # assume only one type of elements and basis   
        for elemPoints in point_sets:
            e = element_types.create_element(elem_types[len(elemPoints)])
    ...
    # ==================================================================#
    def setFitterElementD(self, ep_d):
        ...
        for ei in numpy.sort(list(f.mesh.elements.keys())):
            ...
            elementXis = element.generate_eval_grid(ep_d).squeeze()
    ...
    def _getElementWeights(self, element_number, xi):
        return self.basisWeights[element_number][tuple(xi)]

    def _getFitterElementXis(self, element_number):
        return self.elementXis[element_number]
    ...
    def getElementPointIPerTrueElement(self, GD, sub_meshes):
        """ 
        ...
        if sub_meshes is None:
            sub_meshes = list(self.ensemble_field_function.mesh.elements.keys())
            sub_meshes.sort()
    ...
    def getElementPointINested(self, GD, sub_meshes):
        """
        ...
        # for each subMesh of interest
        for si, s in enumerate(sub_meshes):
            epI.append([])
            subMeshI = 0
            # find starting index of the current subMesh
            for i in range(s):
    ...
    # ==================================================================#
    def makeSubfieldGF(self, elem_i, name=None):
        """
        ...
        try:
            subfield = copy.deepcopy(self.ensemble_field_function.subfields[elem_i])
        except KeyError:
            raise ValueError('subfield ' + str(elem_i) + ' not found')
        else:
            if name is None:
    ...
    def makeGFFromElements(self, name, elements, basis_types):
        """
    ...
    # ==================================================================#
    def makeElementEvaluator(self, element_number):

        class ElementGeometryEvaluator(object):

            def __init__(self, element_, basis, evaluator, parameters):
        ...
        element = self.ensemble_field_function.mesh.elements[element_number]
        ...
        for p in self.field_parameters:
            self.ensemble_field_function.set_parameters(p)
            elemParameters.append(self.ensemble_field_function._get_element_parameters(element_number))
        elemParameters = numpy.array(elemParameters)

        return ElementGeometryEvaluator(element, elemBasis, elemEvaluator, elemParameters)

    def discretiseElementRegularGeoD(self, element_number, max_distance, geo_coords=False):
        """
        ...
        """
        elementEvaluator = self.makeElementEvaluator(element_number)
        xi = discretisation.discretiseRegularGeoD(max_distance, elementEvaluator)
        if geo_coords:
    ...
    def discretiseAllElementsRegularGeoD(self, max_distance, geo_coordinates=False, unpack=True):
        ...
        for elementNumber in numpy.sort(list(self.ensemble_field_function.mesh.elements.keys())):
            if self.ensemble_field_function.mesh.elements[elementNumber].is_element == True:
                elementOutput.append(self.discretiseElementRegularGeoD(elementNumber, max_distance, geo_coords=geo_coordinates))
            else:
                g = self.makeSubfieldGF(elementNumber)
                elementOutput.append(
                    g.discretiseAllElementsRegularGeoD(max_distance, geo_coordinates=geo_coordinates, unpack=unpack))

        xi = [e[0] for e in elementOutput]
        geo = [e[1] for e in elementOutput]

        if geo_coordinates and unpack:
            geo = numpy.vstack(geo)
            xi = numpy.vstack(xi)
        elif not geo_coordinates:
            geo = None
            if unpack:
                xi = numpy.vstack(xi)
    ...
    def generateSurfacePointGrid(self, spacing):
        ...
        dataXi, dataCoords = self.discretiseAllElementsRegularGeoD(spacing, geo_coordinates=True, unpack=False)
    ...
    def triangulate(self, GD, merge=True, ret_vert_map=False):
    ...
    # volume functions                                                   #
    # ==================================================================#
    def isInteriorToSurface(self, points, GD=2.0, max_out_dist=None, exact_search=False):
        """
        ...
        if exact_search:
            GFXi, GFX, closestDist = self.find_closest_material_points(points, init_gd=[80, 80])
            NX = [self.evaluate_normal_in_mesh(None, elemXi={Xi[0]: Xi[1]}).squeeze() for Xi in GFXi]
            NX = numpy.array(NX)
            PX = points - GFX
        else:
            # evaluate points on surface
            GFXi, GFX = self.discretiseAllElementsRegularGeoD(GD, geo_coordinates=True, unpack=False)
        ...
        if max_out_dist is not None:
            if max_out_dist > 0.0:
                ...
                mask = mask | (closestDist < max_out_dist)
            else:
                mask = mask & (closestDist > abs(max_out_dist))

        self.ensemble_field_function = self.ensemble_field_function_old

        return mask

    def generateInternalPointsGrid(self, spacing, max_out_dist=None, exact_closest_search=False):
        """
        ...
        if max_out_dist is None:
            bboxMin = s.min(0)
            bboxMax = s.max(0)
        else:
            bboxMin = s.min(0) - max_out_dist
            bboxMax = s.max(0) + max_out_dist
        ...
        isInterior = self.isInteriorToSurface(PAll, max_out_dist=max_out_dist, exact_search=exact_closest_search)
...
# ======================================================================#
class Spline3dParametric(object):
...
class Spline3dInterpX(object):
    ...
    def eval(self, x):
        ...
        """
        new_y = splev(x, self.tcky)
        new_z = splev(x, self.tckz)
...
def makeGeometricFieldEvaluator(G, eval_d):
    ...
    for elementNumber in numpy.sort(list(f.mesh.elements.keys())):
        ...
        if basisValues.get(element.type) is None:
            if isinstance(eval_d, float):
                evalGrid = G.discretiseElementRegularGeoD(elementNumber, eval_d)[0]
            else:
                evalGrid = element.generate_eval_grid(eval_d).squeeze()
    ...
    def evaluator(P):
        P3 = P.reshape((3, -1, 1))
        V = []
        for element_number in numpy.sort(list(f.mesh.elements.keys())):
            # map global parameters to element parameters
            p1 = f.mapper.get_element_parameters(element_number, P3[0], do_hack=1)
            p2 = f.mapper.get_element_parameters(element_number, P3[1], do_hack=1)
            p3 = f.mapper.get_element_parameters(element_number, P3[2], do_hack=1)
            # combine with static basis values 
            V.append(numpy.dot(basisMatrices[element_number], numpy.vstack((p1, p2, p3)).T))
...
def makeGeometricFieldEvaluatorSparse(G, eval_d, ep_index=None, ep_xi=None, mat_points=None):
    ...
    if mat_points is not None:
        ...
        epMode = 1
    elif isinstance(eval_d, float):
        ep = G.discretiseAllElementsRegularGeoD(eval_d, unpack=False)[0]
        nEPs = numpy.sum([e.shape[0] for e in ep])
        epMode = 1
    else:
        ep = G.evaluate_geometric_field(eval_d)
        nEPs = ep.shape[1]
        epMode = 2
    ...
    if epMode == 3:
        # ~ pdb.set_trace()
        elemEnsNodes = {}
        for mpI, (elem, xi) in enumerate(mat_points):
...
def makeGeometricFieldElementsEvaluatorSparse(gf, elems, eval_d):
    ...
    for elem in elems:
        elemXi = gf.discretiseElementRegularGeoD(
            elem, eval_d, geo_coords=False
        )[0]
        elemXis += [(elem, xi) for xi in elemXi]
    elemsEvaluator = makeGeometricFieldEvaluatorSparse(
        gf, None, mat_points=elemXis
    )
    return elemsEvaluator


def buildEvaluatorSparse(A, entries, node_map, dimensions):
    """
    ...
    row = 0
    for eN, b in entries:
        ensemblePoints = [node_map[eN][n][0][0] for n in range(b.shape[0])]  # element to ensemble map
...
def makeGeometricFieldDerivativesEvaluatorSparse(G, eval_d, dim=3, ep_index=None, ep_xi=None):
    ...
    for ei, elementNumber in enumerate(numpy.sort(list(f.mesh.elements.keys()))):
        ...
        if epMode == 1:
            ...
            if basisValues.get(element.type) is None:
                evalGrid = element.generate_eval_grid(eval_d)
