...
def elemEPList2elemEPDict(elem_ep_list):
    d = {}
    dIndex = {}
    for i, (elem, ep) in enumerate(elem_ep_list):
        try:
            d[elem].append(ep)
        except KeyError:
            d[elem] = [ep, ]
...
def getKeepXi(orig_xi_list, keep_mask):
    """
    ...
    keepList = []
    i = 0
    for l in orig_xi_list:
        keepList.append(keep_mask[i:i + len(l)])
        i += len(l)

    for elem, elemKeep in enumerate(keepList):
        keepXiDict[elem] = numpy.array(orig_xi_list[elem])[numpy.where(elemKeep)]
...
def BIC(n, k, err_var):
    ...
    """
    return n * numpy.log(err_var) + k * numpy.log(n)
