...
def drawBins3d(D, C, bins=None, n_bins=None):
    from mayavi import mlab
    if bins is None:
        bins = histogram(C, n_bins)[1]
    else:
        n_bins = len(bins) - 1

    CBini = digitize(C, bins)
    f = mlab.figure(bgcolor=(0.0, 0.0, 0.0), size=(800, 800))
    mlab.points3d(D[:, 0], D[:, 1], D[:, 2], CBini, mode='point', figure=f, vmin=1.0, vmax=n_bins, scale_factor=0.2, )


def drawPolyMesh(v, f, C, bins=None, n_bins=None):
    from mayavi import mlab
    if bins is None:
        bins = histogram(C, n_bins)[1]
    else:
    ...
    mlab.triangular_mesh(v[:, 0], v[:, 1], v[:, 2], f, figure=fig, scalars=CBini, vmin=1.0, vmax=n_bins)


def matchBins(x, y_pd):
...
def assignBins(a_scalar, b_scalar, n_bins):
    # make bins for aScalar
    aBins = histogram(a_scalar, n_bins)[1]
    bBins = histogram(b_scalar, n_bins)[1]
    # digitise aScalar into aBins
    aBinIndices = digitize(a_scalar, aBins)
    # digitise bScalar into bBins
    bBinIndices = digitize(b_scalar, bBins)
...
def assignBins2(a_scalar, a_bins, b_scalar, b_bins):
    """ assign each point in b the corresponding bin index of a
    """

    # digitise aScalar into aBins
    aBinInd = digitize(a_scalar, a_bins)
    # digitise bScalar into bBins
    bBinInd = digitize(b_scalar, b_bins)
    # get indices of a that belong to the bin of each bScalar
