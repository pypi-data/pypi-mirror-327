...
class MeshEnsemble:
    ...
    def __init__(self, name, dimensions, debug=0):
        ...
        # !! needs to incorporate topological information: xi orientation and element CS density
        self.connectivity = {}  # { ( element_num, point_num, ?? ) : [(element_num, point_num, ??), ...]] }
    ...
    # ==================================================================#
    def save_mesh(self, filename=None, path=None, submesh_filenames=None):
        if not filename:
            filename = self.name
        ...
        return save_mesh_json(
            filename, self, filedir=path, subfieldfns=submesh_filenames
        )

    def save_mesh_shelve(self, filename=None, path='', submesh_filenames=None):
        ...
        for e in list(self.elements.items()):
            if e[1].is_element:
                ...
                if submesh_filenames:
                    element_list.append((e[0], 1, os.path.split(path + submesh_filenames[e[0]])[1]))
                else:
    ...
    def remove_element(self, element_number):
        ...
        # shouldn't decrease since this is used when adding new elements - if a non-last element is removed, new element will overwrite last element ###
        # self.element_counter -= 1
