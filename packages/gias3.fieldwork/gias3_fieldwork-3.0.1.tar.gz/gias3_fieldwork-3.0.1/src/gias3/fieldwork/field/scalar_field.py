...
def loadScalarField(filename, ens_filename=None, mesh_filename=None, path=None):
    if path is not None:
    ...
    try:
        ...
        if ens_filename:
            F = EFF.load_ensemble(ens_filename, meshFilename=mesh_filename, path=path)
        else:
            try:
                F = EFF.load_ensemble(S['ensemble_field'], meshFilename=mesh_filename, path=path)
            except:
...
class scalarField(object):
    ...
    def __init__(self, name, ensemble_field_function, parameters=None):
        self.name = name
        self.ensembleFieldFunction = ensemble_field_function
        self.fieldParameters = None
        self.ensembleFieldFunctionOld = None
    ...
    # ==================================================================#
    def saveScalarField(self, filename, field_filename=None, mesh_filename=None):
        ...
        S['name'] = self.name
        if field_filename:
            S['ensemble_field'] = self.ensembleFieldFunction.save_ensemble(field_filename, mesh_filename=mesh_filename)
        # ~ else:
    ...
    def evaluateFieldAtMaterialPoints(self, matpoints):
        ...
        [(0, [0.5, 0.1]), (1, [0.1, 0.1]),]
        """
        evaluator = makeScalarFieldEvaluatorSparse(self, [1, 1], mat_points=matpoints)
...
# ======================================================================#
def makeScalarFieldEvaluatorSparse(S, eval_d, mat_points=None):
    ...
    if not f.is_flat():
        f = f.flatten()[0]
    ep = S.evaluateField(eval_d)

    if mat_points is not None:
        nEPs = len(mat_points)
    else:
        ep = S.evaluate_geometric_field(eval_d)
        nEPs = ep.shape[1]
...
def makeScalarFieldDerivativesEvaluatorSparse(S, eval_d):
    ...
    ep, epd = S.evaluateField(eval_d, derivs=-1)
...
# ======================================================================#
def fitScalarFieldEPDP(SF, data, eval_d, ftol=None, xtol=None, epsfcn=None, maxit=None):
    if maxit:
        maxfev = maxit * SF.ensembleFieldFunction.get_number_of_ensemble_points()
    else:
        maxfev = 0

    evaluator = makeScalarFieldEvaluatorSparse(SF, eval_d)
    ...
    pOpt = leastsq(obj, SF.fieldParameters.copy().ravel(), ftol=ftol, xtol=xtol, epsfcn=epsfcn, maxfev=maxfev)[0][:, numpy.newaxis]
...
class fitGeomFieldToScalar(object):
    ...
    def __init__(self, G, data_geom, data_scalar):
        self.G = G
        self.dataG = data_geom
        self.dataGTree = None
        self.dataS = data_scalar
        self.xtol = 1e-8
        self.ftol = 1e-8
        self.epsfcn = 0
        self.maxIt = 5

    def initialiseScalarField(self, field_name=None, p0=None):
    ...
    def fitEPDP(self, eval_d):
        ...
        ep = self.G.evaluate_geometric_field(eval_d).T
        ...
        pOpt, SFFit = fitScalarFieldEPDP(self.SF, self.dataS, eval_d, ftol=self.ftol, xtol=self.xtol, epsfcn=self.epsfcn,
                                         maxit=self.maxIt)
        return SFFit

    def fitDPEP(self, eval_d):
        # = DPEP FIT ===========================================================#
        ...
        # fit fC to data curvature
        pOpt, SFFit = fitScalarFieldDPEP(self.SF, self.dataS, eval_d, dpepi, ftol=self.ftol, xtol=self.xtol,
...
def makeMaskedDataObj(S, evalD, maskedData, sobW=None, sobD=None, nW=None, nD=None):
    ...
    if (sobW is None) and (nW is None):
...
def fitGeomFieldToMaskedScalar(G, GD, masked_data, field_name=None, sob_w=None, sob_d=None, n_w=None, n_d=None, xtol=1e-9):
    ...
    """
    fieldFitter = fitGeomFieldToScalar(G, G.evaluate_geometric_field(GD).T, masked_data)
    field = fieldFitter.initialiseScalarField(field_name=field_name)
    fieldObj = makeMaskedDataObj(field, GD, masked_data, sobW=sob_w, sobD=sob_d, nW=n_w, nD=n_d)
