...
from gias3.common.geoprimitives import LineElement3D

# ======================================================================#
from gias3.common.math import norm

log = logging.getLogger(__name__)


class SplineParametric(object):
...
def fitSplineCoeffDPEPObj(x, data, c_shape, spline, n_ep):
    spline.tck[1] = x.reshape(c_shape)
    EP = spline.eval(numpy.linspace(0.0, 1.0, n_ep)).T
    epTree = cKDTree(EP)
...
def fitSplineCoeffEPDPObj(x, c_shape, spline, n_ep, data_tree):
    spline.tck[1] = x.reshape(c_shape)
    EP = spline.eval(numpy.linspace(0.0, 1.0, n_ep)).T
    d = data_tree.query(list(EP))[0]
    # ~ print 'rms', numpy.sqrt( (d*d).mean() )
    return d


def fitSplineCoeffPerDPEPObj(x, data, c_shape, spline, n_ep, k):
    temp = x.reshape(c_shape)
...
def fitSplineCoeffPerEPDPObj(x, c_shape, spline, n_ep, k, data_tree):
    temp = x.reshape(c_shape)
    spline.tck[1] = numpy.hstack((temp, temp[:, :k]))
    EP = spline.eval(numpy.linspace(0.0, 1.0, n_ep)).T
    d = data_tree.query(list(EP))[0]
...
# ======================================================================#
def sortPointsCircular(points, start_i=None):
...
class PointCrawlerImage(object):
    ...
    def trace(self, p1, p2, debug=0):
        ...
        if len(pathLengths) == 0:
            ...
            if len(bestPath) > 3:
                line = SplineParametric(numpy.transpose(bestPath), smoothing=self.smooth, order=3)
                return line, bestPath
            elif len(bestPath) == 3:
                line = SplineParametric(numpy.transpose(bestPath), smoothing=self.smooth, order=2)
                return line, bestPath
            elif len(bestPath) == 2:
...
class RidgeFitter(object):
    ...
    def fit(self, it=3, knots_n=3, order=3, smoothing=3.0):
        ...
        for i in range(it):
            ...
            initial = self._sampleSpline(knots_n)
            ...
            self.splines.append(SplineParametric(numpy.transpose(newKnots), smoothing=smoothing, order=order))
...
# ======================================================================#
def fitCurveEPDP(curve, data, p_init, debug=0):
...
# ======================================================================#
def fitCurveDPEP(curve, data, p_init, debug=0):
    """ fit a 1D lagrange curve to data, fix ends. p0 should of the
    shape (dims,nNodes,paramsPerNode)
    """
    xtol = 1e-3
