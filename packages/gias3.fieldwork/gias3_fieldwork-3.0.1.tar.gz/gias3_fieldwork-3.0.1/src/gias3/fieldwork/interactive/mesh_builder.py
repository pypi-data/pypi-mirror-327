...
from gias3.fieldwork.field.tools import spline_tools, fitting_tools
from gias3.fieldwork.field.topology import element_types
from gias3.fieldwork.interactive.geometric_field_interactor import point_picker_3
...
class Viewer(HasTraits):
    ...
    def _addCurveButton_fired(self):
        ...
        # create curve GF
        cEns = EFF.EnsembleFieldFunction('curve', 1, debug=0)
    ...
    def _picking_done(self, picked_points, mode):
        ...
        if mode == 'addElement':
            self.MB.addElementByPoints(self.element, picked_points)
            self.element = None
        elif mode == 'addCurve':
            self.MB.addCurve(self.curve, picked_points)
            self.curve = None
            ...
            newParams = picked_points[1][:, numpy.newaxis]
...
class MeshBuilder(object):
    ...
    def __init__(self):
        ...
        self.dataScalar = None
        self.crawler = None
        self.curveElemBasis = None
        self.GFFilename = None
        self.ensFilename = None
    ...
    def modifyPoint(self, point_n, new_params):
        # ~ p = numpy.array( newParams[0] )[:,numpy.newaxis]
        p = new_params
        self.GF.modify_geometric_point(point_n, p)
        return

    def movePoint(self, point_n, move):
        p0 = self.GF.get_point_position(point_n)
        p1 = p0 + move
        self.GF.modify_geometric_point(point_n, p1[:, numpy.newaxis])
        return
